...
/* ─────────────────── HERO ASSEMBLE-IN ANIMATION (revert: see HERO-ANIMATION-NOTES.md) ─────────────────── */
const RevealContext = React.createContext(false);
function RevealSection({ storageKey, threshold = 0.35, children, style }) {
  const ref = React.useRef(null);
  const [played, setPlayed] = useState(() => { try { return sessionStorage.getItem(storageKey) === '1'; } catch (e) { return false; } });
  useEffect(() => {
    if (played) return;
    const el = ref.current;
    if (!el) return;
    const reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (reduce) { setPlayed(true); try { sessionStorage.setItem(storageKey, '1'); } catch (e) {} return; }
  ...
  return (
    <div ref={ref} style={style}>
      <RevealContext.Provider value={played}>{children}</RevealContext.Provider>
    </div>
  );
}
function SlideInLine({ index = 0, children, style }) {
...
/* Fades + slides an element up once, the first time it scrolls into view. Generic — used on the
   About page (revert: see PAGE-ANIMATIONS-NOTES.md). */
function ScrollReveal({ children, style, threshold = 0.15 }) {
  const ref = React.useRef(null);
...
/* Fades + slides an element up once on mount (page load), not on scroll — for content already
   visible above the fold, e.g. the contact page. */
function MountReveal({ index = 0, children, style }) {
...
/* Section wrapper that fades/slides up into view once, used for scroll-reveal on the menu page (revert: see MENU-PAGE-ENHANCEMENTS-NOTES.md). */
function MenuSectionReveal({ sectionId, sectionRefs, style, children }) {
  const ref = React.useRef(null);
  const [visible, setVisible] = useState(false);
  useEffect(() => {
    const el = ref.current;
  ...
  return (
    <div
      ...
      ref={el => { ref.current = el; if (sectionRefs) sectionRefs.current[sectionId] = el; }}
      style={{
        ...style,
        opacity: visible ? 1 : 0,
...
function AssembleImage({ src, alt }) {
  const played = React.useContext(RevealContext);
  return (
    <div style={{ position: 'relative', width: '100%', overflow: 'hidden' }}>
      <img src={src} alt={alt} loading="lazy" decoding="async" style={{
        width: '100%', height: 'auto', objectFit: 'contain', objectPosition: 'center', display: 'block', mixBlendMode: 'multiply',
        opacity: played ? 1 : 0,
...
function useViewport() {
  ...
  return {
    width: w,
    isPhone:   w <= 480,                 // narrow phone (iPhone SE)
    isMobile:  w <= 768,                 // any phone, portrait tablet narrow
    isTablet:  w > 768 && w <= 1024,     // tablet, small laptop
    isDesktop: w > 1024,
  };
}

/* ─────────────────── CONTATTI ───────────────────
   CONTACT_EMAIL: la casella de la botéga, mostrada in ciaro par chi vol scrìvar da so posta.
   CONTACT_FORM_KEY: ciave de Web3Forms (web3forms.com, gratis). Finché la xe voda, el
   modulo el verze el programa de posta del visitador; co la ciave, el manda da solo. */
...
/* ─────────────────── DIALECT MARQUEE ─────────────────── */
const DIALECT_SAYINGS = [
  "El pan del dì de prima l'è un baso de bon Dio.",
  "A catar bota pì che el pan!",
  "Tuti i poenta e bocal.",
  "Ciapar el musso par la coa.",
  "Mangia che te fa belo, bevi che te fa tondo.",
  "El vin el fa bon sangue, ma l'aqua la fa la mufa.",
  "Vestite ben che i te speta, va in camisa che i te conose.",
  "El tempo l'è come le done: el fa quel che vol lù.",
  "El lupo el perde el peo, ma no el vizio.",
  "A verzer la boca se fa pì presto che a serarla.",
  "Ciapa su e porta casa.",
  "Esser come i osei de Padoa: tanta vose e gnente sostanza.",
  "Fé el passo secondo la gamba.",
  "Pian pianin se va lontan.",
  "Tuti i conti i torna, ma i zeri i pesa.",
  "No tor via el tempo al bon tempo.",
  "Se no l'è lu, l'è so fradèlo.",
  "Va a ciapar le senzale.",
  "L'è un omo da strapazo.",
  "O vecia o nova, la poenta la m'inova.",
];
function DialectMarquee() {
  const { lang } = useLang();
  const vp = useViewport();
  if (lang !== 'vec') return null;
  const items = DIALECT_SAYINGS.concat(DIALECT_SAYINGS); // duplicato per lo scorrimento continuo
  return (
    <div className="dialect-marquee-strip" role="marquee" aria-label="Modi de dire polesani" style={{
      background: '#1A1108', borderBottom: '2px solid #D4A640', overflow: 'hidden', position: 'relative',
    }}>
      <div className="dialect-marquee-track" style={{ padding: vp.isMobile ? '8px 0' : '9px 0' }}>
        {items.map((s, i) => (
          <span key={i} style={{ display: 'inline-flex', alignItems: 'center', flexShrink: 0 }}>
            <span style={{ color: '#D4A640', fontSize: vp.isMobile ? 12 : 13, margin: vp.isMobile ? '0 14px' : '0 20px' }} aria-hidden="true">◆</span>
            <span style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: vp.isMobile ? 13 : 15, color: '#F5F0E6', letterSpacing: '0.01em' }}>{s}</span>
          </span>
        ))}
      </div>
    </div>
...
/* ─────────────────── LANG BAR ─────────────────── */
function LangBar({ onNavigate }) {
  const vp = useViewport();
  const { lang, setLang } = useLang();
  const [showCallout, setShowCallout] = useState(() => { try { return lang === 'it' && !sessionStorage.getItem('piva_dialect_callout_seen'); } catch (e) { return false; } });
  const dismissCallout = () => { setShowCallout(false); try { sessionStorage.setItem('piva_dialect_callout_seen', '1'); } catch (e) {} };
  return (
    <div className="notranslate" translate="no" style={{ background: '#FDFAF4', borderBottom: '1px solid rgba(61,43,26,0.10)', padding: vp.isMobile ? '6px 14px' : '7px 40px', display: 'flex', justifyContent: 'center', position: 'relative' }}>
      <div style={{ display: 'flex', gap: 6, maxWidth: 1200, width: '100%', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: vp.isMobile ? 8 : 10, position: 'relative' }}>
          {[{ code: 'it', flag: '🇮🇹', label: 'Italiano', short: 'IT' }, { code: 'en', flag: '🇬🇧', label: 'English', short: 'EN' }, { code: 'vec', label: 'Rovigòto (dialèto)', short: 'Rovigoto', svg: true }].map(({ code, flag, label, short, svg }) => (
            <button key={code} onClick={() => { if (code === 'vec') { dismissCallout(); if (lang === 'vec') { window.location.href = 'rovigoto/index.html'; return; } } setLang(code); }} aria-label={label} title={code === 'vec' && lang === 'vec' ? 'Vai al dizionario Rovigòto' : label} style={{
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2,
              background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, flexShrink: 0,
              position: 'relative',
            }}>
              {code === 'vec' && showCallout && (
                <div role="tooltip" style={{
                  position: 'absolute', top: '50%', left: '100%', marginLeft: 10, transform: 'translateY(-50%)', width: 168,
                  background: '#C8251D', color: '#FDFAF4', borderRadius: 10, padding: '10px 12px',
                  fontFamily: 'var(--font-body)', fontSize: 11.5, lineHeight: 1.5, fontWeight: 400,
                  boxShadow: '0 8px 24px rgba(200,37,29,0.32)', zIndex: 50, whiteSpace: 'normal', textAlign: 'left',
                  animation: 'dialect-invite 2.2s ease-in-out infinite',
                }}>
                  Parli Rovigòto? Prova il sito nel nostro dialetto.
                  <div style={{ position: 'absolute', top: '50%', right: '100%', transform: 'translateY(-50%)', width: 0, height: 0, borderTop: '6px solid transparent', borderBottom: '6px solid transparent', borderRight: '6px solid #C8251D' }} />
                </div>
              )}
              {code === 'vec' && !showCallout && lang === 'vec' && (
                <div role="tooltip" style={{
                  position: 'absolute', top: '50%', left: '100%', marginLeft: 10, transform: 'translateY(-50%)', width: 176,
                  background: '#D4A640', color: '#1A1108', borderRadius: 10, padding: '10px 12px',
                  fontFamily: 'var(--font-body)', fontSize: 11.5, lineHeight: 1.5, fontWeight: 600,
                  boxShadow: '0 8px 24px rgba(212,166,64,0.38)', zIndex: 50, whiteSpace: 'normal', textAlign: 'left',
                  display: 'flex', alignItems: 'center', gap: 7,
                  animation: 'dialect-invite 2.2s ease-in-out infinite',
                }}>
                  <span>Struca ancora: va al dizionario</span>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#1A1108" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M5 12h13M13 6l6 6-6 6" /></svg>
                  <div style={{ position: 'absolute', top: '50%', right: '100%', transform: 'translateY(-50%)', width: 0, height: 0, borderTop: '6px solid transparent', borderBottom: '6px solid transparent', borderRight: '6px solid #D4A640' }} />
                </div>
              )}
              <span style={{
                width: vp.isMobile ? 28 : 30, height: vp.isMobile ? 28 : 30,
                ...
                borderRadius: '50%', transition: 'all 0.2s ease', lineHeight: 1,
              }}>{svg ? (
                <img src="images/brand/flag_rovigo.png" alt="" aria-hidden="true" style={{ width: '86%', height: '86%', objectFit: 'contain', display: 'block' }} />
              ) : flag}</span>
              <span style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 8 : 9, fontWeight: 700, letterSpacing: '0.06em', color: lang === code ? '#C8251D' : '#8A7A63' }}>{short}</span>
            </button>
          ))}
        </div>
        <button onClick={() => onNavigate('ricette')} style={{
          background: '#C8251D', color: '#fff',
          fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 12 : 13, fontWeight: 600,
          padding: vp.isMobile ? '7px 14px' : '9px 20px',
          borderRadius: 999, border: 'none', cursor: 'pointer',
          letterSpacing: '0.04em', boxShadow: '0 2px 8px rgba(200,37,29,0.25)',
          transition: 'all 0.25s ease', flexShrink: 0, whiteSpace: 'nowrap',
        }}
          onMouseEnter={e => { e.currentTarget.style.background='#9E1C16'; e.currentTarget.style.transform='translateY(-1px)'; }}
          onMouseLeave={e => { e.currentTarget.style.background='#C8251D'; e.currentTarget.style.transform='translateY(0)'; }}
        >{lang === 'vec' ? 'Ricéte' : lang === 'en' ? 'Recipes' : 'Ricette'}</button>
      </div>
    </div>
  );
}
...
function Header({ activePage, onNavigate }) {
  ...
  const links = lang === 'vec'
    ? [
        { id: 'home', label: 'Casa' },
        { id: 'menu', label: vp.isPhone ? 'Menù' : 'El Menù' },
        { id: 'about', label: 'Chi Sémo' },
        { id: 'contact', label: 'Contàti' },
      ]
    : lang === 'en'
    ? [
        { id: 'home', label: 'Home' },
    ...
    : [
        { id: 'home', label: 'Home' },
        { id: 'menu', label: vp.isPhone ? 'Menù' : 'Il Menù' },
  ...
  return (
    <header style={{
      ...
      <div style={{
        ...
        <div onClick={() => onNavigate('home')} style={{ display: 'flex', alignItems: 'center', gap: vp.isMobile ? 8 : 12, cursor: 'pointer', minWidth: 0 }}>
          <img src={window.IMGS.logoSvg} alt="F.lli Piva" loading="eager" decoding="async" style={{ width: vp.isMobile ? 56 : 88, height: vp.isMobile ? 31 : 49, borderRadius: 0, objectFit: 'contain', flexShrink: 0 }} />
        ...
        <nav style={{ display: 'flex', gap: vp.isMobile ? 12 : vp.isTablet ? 16 : 28, alignItems: 'center', flexShrink: 0 }}>
          {links.map(l => (
            <button key={l.id} onClick={() => onNavigate(l.id)} style={{
              background: 'none', border: 'none', cursor: 'pointer',
              fontFamily: 'var(--font-body)', fontSize: vp.isPhone ? 11 : vp.isMobile ? 12 : 14,
              fontWeight: activePage === l.id ? 600 : 400,
              color: activePage === l.id ? '#C8251D' : '#3D2B1A',
              paddingBottom: 2, padding: vp.isMobile ? '4px 2px' : undefined,
...
/* ─────────────────── ORARI: dati condivisi (hero badge + pagina contatti) ─────────────────── */
const DAY_NAMES_IT = ['Domenica','Lunedì','Martedì','Mercoledì','Giovedì','Venerdì','Sabato'];

const HOURS_ESTATE = [
  { day: 'Lunedì',     times: ['8:00 – 13:00', '17:00 – 20:00'] },
  { day: 'Martedì',    times: ['8:00 – 13:00', '17:00 – 20:00'] },
  { day: 'Mercoledì',  times: ['8:00 – 13:00'], note: 'Pomeriggio chiuso' },
  { day: 'Giovedì',    times: ['8:00 – 13:00', '17:00 – 20:00'] },
  { day: 'Venerdì',    times: ['8:00 – 13:00', '17:00 – 20:00'] },
  { day: 'Sabato',     times: ['8:00 – 13:00', '17:00 – 20:00'] },
  { day: 'Domenica',   times: ['Chiuso'], closed: true },
];
const HOURS_INVERNO = [
  { day: 'Lunedì',     times: ['8:00 – 13:00', '16:30 – 19:30'] },
  { day: 'Martedì',    times: ['8:00 – 13:00', '16:30 – 19:30'] },
  { day: 'Mercoledì',  times: ['8:00 – 13:00'], note: 'Pomeriggio chiuso' },
  { day: 'Giovedì',    times: ['8:00 – 13:00', '16:30 – 19:30'] },
  { day: 'Venerdì',    times: ['8:00 – 13:00', '16:30 – 19:30'] },
  { day: 'Sabato',     times: ['8:00 – 13:00', '16:30 – 19:30'] },
  { day: 'Domenica',   times: ['Chiuso'], closed: true },
];
...
const HOLIDAYS_BASE = [
  { name: 'Capodanno', date: '1 gennaio' },
  { name: 'Epifania', date: '6 gennaio' },
  { name: 'Pasqua e Pasquetta', variableEaster: true },
  { name: 'Festa della Liberazione', date: '25 aprile' },
  { name: 'Festa del Lavoro', date: '1 maggio' },
  { name: 'Festa della Repubblica', date: '2 giugno' },
  { name: 'Ferragosto', date: '15 agosto' },
  { name: 'Ognissanti', date: '1 novembre' },
  { name: 'Immacolata Concezione', date: '8 dicembre' },
  { name: 'Natale', date: '25 dicembre' },
  { name: 'Santo Stefano', date: '26 dicembre' },
];
const HOLIDAY_NAME_EN = {
  'Capodanno': "New Year's Day", 'Epifania': 'Epiphany', 'Pasqua e Pasquetta': 'Easter & Easter Monday',
  'Festa della Liberazione': 'Liberation Day', 'Festa del Lavoro': 'Labour Day', 'Festa della Repubblica': 'Republic Day',
  'Ferragosto': 'Ferragosto (Assumption Day)', 'Ognissanti': "All Saints' Day", 'Immacolata Concezione': 'Immaculate Conception',
  'Natale': 'Christmas', 'Santo Stefano': "St. Stephen's Day", 'Pasqua': 'Easter', 'Pasquetta': 'Easter Monday',
...
/* Sostituisce la voce "Pasqua e Pasquetta" con le date effettive della Pasqua
   in corso o della prossima (se quella di quest'anno è già passata). */
function getHolidaysList(today, lang) {
  ...
  const easterLabel = sameMonth
    ? `${easter.getDate()} – ${fmt(easterMonday)} ${easterMonday.getFullYear()}`
    : `${fmt(easter)} – ${fmt(easterMonday)} ${easterMonday.getFullYear()}`;
  const list = HOLIDAYS_BASE.map(h => h.variableEaster ? { name: h.name, date: easterLabel } : { ...h, date: lang === 'en' ? (HOLIDAY_DATE_EN[h.name] || h.date) : h.date });
  ...
  const extra = CHIUSURE_STRAORDINARIE.filter(c => today0 <= closureRange(c)[1]).map(c => {
    ...
    return { name: c.name, date: lang === 'en' ? `${MONTH_NAMES_EN[s.getMonth()]} ${s.getDate()}–${e.getDate()}, ${e.getFullYear()}` : label };
  });
  return [...extra, ...list];
}
const HOLIDAY_NAME_VEC = {
  'Capodanno': 'Ano Novo', 'Epifania': 'Befana', 'Pasqua e Pasquetta': 'Pasqua e Pasquéta',
  'Festa della Liberazione': 'Festa de la Liberassión', 'Festa del Lavoro': 'Festa del Laoro', 'Festa della Repubblica': 'Festa de la Repùblica',
  'Ferragosto': 'Ferragosto', 'Ognissanti': 'Tuti i Santi', 'Immacolata Concezione': 'Immacolata Consessión',
  'Natale': 'Nadal', 'Santo Stefano': 'Santo Stèfano', 'Pasqua': 'Pasqua', 'Pasquetta': 'Pasquéta',
  'Chiusura estiva': 'Ferie de l\'istà',
};
/* Festività a data fissa (mese, giorno). Pasqua/Pasquetta si calcolano a parte con getEasterDate. */
const HOLIDAY_DATE_EN = {
  'Capodanno': 'January 1', 'Epifania': 'January 6', 'Festa della Liberazione': 'April 25',
  'Festa del Lavoro': 'May 1', 'Festa della Repubblica': 'June 2', 'Ferragosto': 'August 15',
  'Ognissanti': 'November 1', 'Immacolata Concezione': 'December 8', 'Natale': 'December 25', 'Santo Stefano': 'December 26',
};
...
/* ── CHIUSURE STRAORDINARIE (una tantum) ──
   Chiusure non ricorrenti: ferie, lavori, ecc. Cancellare la voce quando è passata.
   Formato: { name, start: [anno, mese(1-12), giorno], end: [...] } — estremi inclusi. */
const CHIUSURE_STRAORDINARIE = [
  { name: 'Chiusura estiva', start: [2026, 8, 12], end: [2026, 8, 15] },
];
...
/* Restituisce il nome della festività se `date` cade in un giorno di chiusura, altrimenti null. */
...
/* Estate: 1 maggio – 30 settembre. Inverno: 1 ottobre – 31 marzo, esclusi i giorni di dicembre
   (che hanno un orario esteso a parte, vedi HOURS_DECEMBER). */
/* Ultima domenica di un mese (regola UE del cambio ora legale/solare). */
function getLastSundayOfMonth(year, month) {
  // month: 0-based (2 = marzo, 9 = ottobre)
...
/* Unica lista dei mesi in rovigòto: ogni altro punto del sito deriva da qui
   (VEC_MONTHS negli orari e monthNames nel calendario), così non si può più
   correggere un elenco e dimenticare l'altro. */
const MONTH_NAMES_VEC_CAP = MONTH_NAMES_VEC.map(m => m.charAt(0).toUpperCase() + m.slice(1));
/* Testo che spiega che "estivo/invernale" seguono il cambio dell'ora legale (UE),
   con le date effettive dell'anno corrente calcolate da getLastSundayOfMonth.
   L'orario estivo va dall'ultima domenica di marzo a quella di ottobre dello stesso anno;
   l'orario invernale va dall'ultima domenica di ottobre a quella di marzo dell'anno successivo. */
function getDstRangeLabel(year, seasonKind, lang) {
  const months = lang === 'en' ? MONTH_NAMES_EN : lang === 'vec' ? MONTH_NAMES_VEC : MONTH_NAMES_IT;
  const fmt = d => `${d.getDate()} ${months[d.getMonth()]}`;
  if (seasonKind === 'inverno') {
    const start = getLastSundayOfMonth(year, 9);        // ultima domenica di ottobre
    const end = getLastSundayOfMonth(year + 1, 2);      // ultima domenica di marzo dell'anno successivo
    return lang === 'en'
      ? `from the last Sunday of October (${fmt(start)}) to the last Sunday of March ${year + 1} (${fmt(end)})`
      : `dall'ultima domenica di ottobre (${fmt(start)}) all'ultima domenica di marzo ${year + 1} (${fmt(end)})`;
  }
  const start = getLastSundayOfMonth(year, 2);  // ultima domenica di marzo
  const end = getLastSundayOfMonth(year, 9);    // ultima domenica di ottobre
  return lang === 'en'
    ? `from the last Sunday of March (${fmt(start)}) to the last Sunday of October (${fmt(end)})`
    : `dall'ultima domenica di marzo (${fmt(start)}) all'ultima domenica di ottobre (${fmt(end)})`;
}
/* Etichetta breve "DD mese – DD mese AAAA" per il periodo estivo/invernale in corso
   (o il prossimo, se quello attuale è già terminato) — usata accanto al titolo
   "Alcuni Sapori di Estate/Inverno" nel menù. Si aggiorna da sola ogni anno perché
   le date derivano sempre da getLastSundayOfMonth. */
...
/* Estate/Inverno seguono il cambio ora legale (UE): l'orario estivo comincia
   l'ultima domenica di marzo e finisce l'ultima domenica di ottobre.
   Dicembre ha sempre il proprio orario esteso, indipendentemente dall'ora legale. */
function getHoursSeasonForDate(date) {
  const year = date.getFullYear();
  const summerStart = getLastSundayOfMonth(year, 2);  // ultima domenica di marzo
  const winterStart = getLastSundayOfMonth(year, 9);  // ultima domenica di ottobre
  const d0 = new Date(year, date.getMonth(), date.getDate());
  if (d0 >= summerStart && d0 < winterStart) return 'estate';
  return 'inverno';
}
/* Periodo di Carnevale (solo per l'etichetta nel calcolatore orari): dal 6 gennaio
   (Epifania) al Martedì Grasso incluso, come per le stagioni del menù. */
...
/* Orario di oggi: tiene conto di stagione e festività. */
function getTodayHoursInfo(date) {
  const holidayName = getHolidayNameForDate(date);
  if (holidayName) return { closed: true, holidayName };
  ...
  return { closed: !!(entry && entry.closed), entry, seasonKey };
}

/* ─────────────────── HERO ─────────────────── */
function TodayHoursBadge({ onNavigate, isMobile }) {
  ...
  if (lang === 'vec') {
    ...
    dateLabel = now.toLocaleDateString(lang === 'en' ? 'en-GB' : 'it-IT', { weekday: 'long', day: 'numeric', month: 'long' });
  }
  ...
  return (
    <div onClick={() => onNavigate('contact', 'hours-panel')} style={{
      position: 'absolute', top: isMobile ? 14 : 24, right: isMobile ? 14 : 24, zIndex: 3,
      background: 'rgba(26,17,8,0.55)', backdropFilter: 'blur(6px)', border: '1px solid rgba(253,250,244,0.25)',
      borderRadius: 12, padding: isMobile ? '8px 12px' : '10px 16px', cursor: 'pointer', textAlign: 'right',
      transition: 'background 0.2s ease',
    }}
      onMouseEnter={e => { e.currentTarget.style.background = 'rgba(26,17,8,0.72)'; }}
      onMouseLeave={e => { e.currentTarget.style.background = 'rgba(26,17,8,0.55)'; }}
    >
      <div style={{ fontFamily: 'var(--font-body)', fontSize: isMobile ? 10.5 : 11.5, color: '#C9A97A', fontStyle: 'italic', letterSpacing: '0.01em' }}>{dateLabelCap}</div>
      <div style={{ fontFamily: 'var(--font-body)', fontSize: isMobile ? 12 : 13.5, fontWeight: 600, color: info.closed ? '#E8A0A0' : '#FDFAF4', marginTop: 2, whiteSpace: 'nowrap' }}>{statusLabel}</div>
    </div>
  );
}
...
function Hero({ onNavigate }) {
  ...
  const { lang, setLang } = useLang();
  return (
    <section style={{ position: 'relative', height: vp.isPhone ? 380 : vp.isMobile ? 440 : 560, overflow: 'hidden' }}>
      <img src={window.IMGS.storefront} alt="Gastronomia F.lli Piva" loading="eager" decoding="async" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 30%' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(26,17,8,0.72) 0%, rgba(26,17,8,0.55) 60%, rgba(26,17,8,0.30) 100%)' }} />
      <TodayHoursBadge onNavigate={onNavigate} isMobile={vp.isMobile} />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', padding: vp.isPhone ? '0 20px' : vp.isMobile ? '0 28px' : '0 80px', maxWidth: 1200, margin: '0 auto', left: 0, right: 0 }}>
        <div style={{ maxWidth: 520 }}>
          ...
          <div style={{ display: 'flex', gap: vp.isPhone ? 8 : 12, flexWrap: 'wrap' }}>
            <button onClick={() => onNavigate('menu')} style={{
              ...
              padding: vp.isMobile ? '11px 22px' : '13px 28px', borderRadius: 999, border: 'none', cursor: 'pointer',
              letterSpacing: '0.04em', boxShadow: '0 4px 20px rgba(200,37,29,0.40)', transition: 'all 0.25s ease',
            }}
              onMouseEnter={e => { e.currentTarget.style.transform='translateY(-2px)'; e.currentTarget.style.boxShadow='0 6px 28px rgba(200,37,29,0.50)'; }}
              onMouseLeave={e => { e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.boxShadow='0 4px 20px rgba(200,37,29,0.40)'; }}
            >{lang === 'vec' ? 'Varda el menù' : lang === 'en' ? 'Discover our menu' : 'Scopri il menù'}</button>
            <button onClick={() => onNavigate('about')} style={{
              background: 'transparent', color: '#FDFAF4', fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 13 : 14, fontWeight: 500,
              padding: vp.isMobile ? '10px 18px' : '12px 24px', borderRadius: 999, border: '1.5px solid rgba(253,250,244,0.50)', cursor: 'pointer', transition: 'all 0.25s ease',
            }}
              onMouseEnter={e => { e.currentTarget.style.background='rgba(253,250,244,0.12)'; e.currentTarget.style.borderColor='rgba(253,250,244,0.8)'; }}
              onMouseLeave={e => { e.currentTarget.style.background='transparent'; e.currentTarget.style.borderColor='rgba(253,250,244,0.50)'; }}
            >{lang === 'vec' ? 'Chi sémo' : lang === 'en' ? 'About Us' : 'Chi siamo'}</button>
          </div>
        </div>
      </div>
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 80, background: 'linear-gradient(to top, #F5F0E6, transparent)' }} />
    </section>
  );
}
...
const MENU_DISHES = [
  ...
  { name: 'Vitello Tonnato', src: 'images/dishes/vitello_tonnato.jpg' },
  { name: 'Orecchiette', src: 'images/dishes/orecchiette.jpg', season: 'estate' },
  { name: 'Pasta Pomodorini Cacio e Pepe', src: 'images/dishes/pasta_pomodorini_cacio_e_pepe.jpg', season: 'estate' },
  { name: 'Pomodori Ripieni', src: 'images/dishes/pomodori_ripieni.jpg', season: 'estate' },
  { name: 'Insalata Russa', src: 'images/dishes/insalata_russa.jpg' },
  { name: 'Pasta Radicchio & Salsiccia', src: 'images/dishes/pasta_radicchio_salsiccia.jpg', season: 'estate' },
  { name: 'Pasta Fredda', src: 'images/dishes/pasta_fredda.jpg', season: 'estate' },
  { name: 'Insalata di Pasta',           src: 'images/dishes/insalata_di_pasta.jpg', season: 'estate' },
  { name: 'Pasta Italia',              src: 'images/dishes/pasta_italia.jpg', season: 'estate' },
  { name: 'Insalata di Riso',         src: 'images/dishes/insalata_di_riso.jpg', season: 'estate' },
  { name: 'Insalata di Farro',        src: 'images/dishes/insalata_di_farro.jpg', season: 'estate' },
  { name: 'Farro Vegano',             src: 'images/dishes/farro_vegano.jpg', season: 'estate' },
  { name: 'Insalata di Mare',         src: 'images/dishes/insalata_di_mare.jpg' },
  { name: 'Gnocchi di Zucca',         src: 'images/dishes/gnocchi_di_zucca.jpg', season: 'inverno' },
  { name: 'Cipolle al Forno',         src: 'images/dishes/cipolle_al_forno.jpg', season: 'estate' },
  { name: 'Pollo alla Piastra',       src: 'images/dishes/pollo_alla_piastra.jpg' },
  { name: 'Cappelletti Mostarda',     src: 'images/dishes/cappelletti_mostarda.jpg', season: 'carnevale' },
  { name: 'Porchetta di Nostra Produzione', src: 'images/dishes/porchetta_nostra_produzione.jpg' },
  { name: 'Scaloppine di Pollo al Marsala', src: 'images/dishes/scaloppine_pollo_marsala.jpg' },
  { name: 'Cioccolato Laica',         src: 'images/dishes/laica_cioccolato.jpg' },
  { name: 'Pane di Semola Forte',     src: 'images/dishes/pane_di_semola_forte.jpg' },
  { name: 'Taralli Danieli',          src: 'images/dishes/taralli_danieli.jpg' },
  { name: 'Lasagne Carne',            src: 'images/dishes/lasagne_carne.jpg' },
  { name: 'Lasagne con Asparagi',     src: 'images/dishes/lasagne_con_asparagi.jpg' },
  { name: 'Roast Beef', src: 'images/dishes/roast_beef.jpg' },
  { name: 'Spiedi di Gamberone', src: 'images/dishes/spiedi_di_gamberone.jpg' },
  { name: 'Involtini Valdostani', src: 'images/dishes/involtini_valdostani.jpg' },
  { name: 'Zucca al Forno',           src: 'images/dishes/zucca_al_forno.jpg', season: 'inverno' },
  { name: "Fagiolini con l'Occhio e Acciughine", src: 'images/dishes/fagiolini_occhio_acciughine.jpg' },
  { name: 'Verza con Pancetta',       src: 'images/dishes/verza_con_pancetta.jpg', season: 'inverno' },
  { name: 'Trippa alla Parmigiana',   src: 'images/dishes/trippa_alla_parmigiana.jpg' },
  { name: 'Involtini di Tacchino',    src: 'images/dishes/involtini_di_tacchino.jpg' },
  { name: 'Canederli con Speck',      src: 'images/dishes/canederli_con_speck.jpg', season: 'inverno' },
  { name: 'Baccalà alla Vicentina',   src: 'images/dishes/baccala_alla_vicentina.jpg', season: 'inverno' },
  { name: "Sciroppo per l'inverno",              src: "images/dishes/sciroppo_per_linverno.jpg" },
  { name: "Tonno di Carloforte",              src: "images/dishes/tonno_di_carloforte.jpg" },
  { name: "Babbi Dolcetorta al pistacchio",              src: "images/dishes/babbi_dolcetorta_al_pistacchio.jpg" },
  { name: 'Cioccolato Origine',              src: "images/dishes/cioccolato_origine.jpg" },
  { name: 'Prosciutto di Parma 24 Mesi', src: 'images/dishes/prosciutto_di_parma_24_mesi.jpg' },
  { name: 'Prosciutto di San Daniele Magnum', src: 'images/dishes/prosciutto_san_daniele_magnum.jpg' },
  { name: 'Frutta in Acquavite Prunotto', src: 'images/dishes/frutta_acquavite_prunotto.jpg' },
  { name: 'Riso Acquerello 8 Anni', src: 'images/dishes/riso_acquerello_8_anni.jpg' },
  { name: 'Pinzimonio di Verdure Brezzo', src: 'images/dishes/pinzimonio_di_verdure_brezzo.jpg' },
  { name: 'Specialità di Grano', src: 'images/dishes/specialita_di_grano.jpg' },
  { name: 'Tiramisù della casa', src: "images/dishes/tiramisu_della_casa.jpg" },
  { name: 'Zuppa Inglese', src: 'images/dishes/zuppa_inglese.jpg' },
  { name: 'Porcini Tagliati Testa Nera', src: "images/dishes/porcini_tagliati_testa_nera.jpg" },
  { name: 'Crema Zabaione con Marsala',  src: "images/dishes/crema_zabaione_con_marsala.jpg" },
  { name: 'Aceto Balsamico IGP Campari', src: "images/dishes/aceto_balsamico_igp_campari.jpg" },
  { name: 'Aceto Balsamico Oro Gold',    src: "images/dishes/aceto_balsamico_oro_gold.webp" },
  { name: 'Confettura Extra Mistilli',   src: "images/dishes/confettura_extra_mistilli.jpg" },
  { name: 'Albicocche in Grappa Brezzo', src: "images/dishes/albicocche_in_grappa_brezzo.jpg" },
  { name: 'Distillati Of Bonollo 1908', src: "images/dishes/grappe_of_bonollo.jpg" },
  { name: 'Fusilli di Pisa Martelli', src: "images/dishes/fusilli_di_pisa_martelli.jpg" },
  { name: 'Ragù di Cervo Regi', src: "images/dishes/ragu_di_cervo_regi.jpg" },
  { name: 'Grissini Il Panificio', src: "images/dishes/grissini_il_panificio.jpg" },
  { name: 'Prosecco Astoria & Fongaro', src: "images/dishes/prosecco_astoria_fongaro.jpg" },
  { name: 'Ciambelline Bergamini', src: "images/dishes/ciambelline_ferraresi_bergamini.jpg" },
  ...
  { name: 'Riso Venere', src: 'images/dishes/riso_venere.jpg', season: 'estate' },
  { name: 'Coniglio in Salmì', src: 'images/dishes/coniglio_in_salmi.jpg' },
  { name: 'Polpettone di Vitello', src: 'images/dishes/arrosto_di_vitello_al_forno.jpg' },
  { name: 'Tacchino Farcito', src: 'images/dishes/tacchino_farcito.jpg' },
  { name: 'Arrosto di Vitello al Forno', src: 'images/dishes/arrosto_di_maiale_al_forno.jpg' },
  { name: 'Brasato al Vino', src: 'images/dishes/brasato_al_vino.jpg' },
  { name: 'Pollo con Patate', src: 'images/dishes/pollo_con_patate.jpg' },
  { name: 'Gnocchi alla Romana', src: 'images/dishes/gnocchi_alla_romana.jpg' },
  { name: 'Gnocchi alla Sorrentina', src: 'images/dishes/gnocchi_alla_sorrentina.jpg' },
  { name: 'Pasticciata', src: 'images/dishes/pasticciata.png' },
  { name: 'Stoccafisso', src: 'images/dishes/stoccafisso.jpg', season: 'inverno' },
  { name: 'Fondi di Carciofi', src: 'images/dishes/fondi_di_carciofi.jpg' },
  { name: 'Puré di Patate', src: 'images/dishes/pure_di_patate.jpg' },
  { name: 'Patate al Rosmarino al Forno', src: 'images/dishes/patate_al_rosmarino_al_forno.jpg' },
  { name: 'Salmone agli Agrumi', src: 'images/dishes/tranci_di_salmone.jpg' },
  { name: 'Capesante', src: 'images/dishes/capesante.jpg' },
  { name: 'Bistecca di Tonno', src: 'images/dishes/bistecca_di_tonno.jpg' },
  { name: 'Piselli', src: 'images/dishes/piselli.jpg' },
  { name: 'Spinaci', src: 'images/dishes/spinaci.jpg' },
  { name: 'Prodotti Tipici Rovigo', src: 'images/dishes/prodotti_tipici_rovigo.jpg' },
  { name: 'Selezione Champagne e Spumanti', src: 'images/dishes/selezione_champagne_spumanti.jpg' },
  { name: 'Bellavista Alma Grande Cuvée Brut', src: 'images/dishes/bellavista_alma_brut.jpg' },
  { name: "Foglie d'Ulivo agli Spinaci", src: 'images/dishes/foglie_ulivo_spinaci.jpg' },
  { name: 'Torronfetta Barbero', src: 'images/dishes/torronfetta_barbero.jpg' },
  { name: 'Marmellata di Limoni di Sicilia', src: 'images/dishes/marmellata_limoni_sicilia.jpg' },
  { name: 'Confettura di Mirtilli Brezzo', src: 'images/dishes/confettura_mirtilli_brezzo.jpg' },
  { name: "Miele all'Arancio Brezzo", src: 'images/dishes/miele_arancio_brezzo.jpg' },
  ...
  { name: 'Olio Extra Vergine di Oliva Biologico Salvagno', src: 'images/dishes/olio_evo_biologico_salvagno.jpg' },
  { name: 'Fegato alla Veneziana', src: 'images/dishes/fegato_alla_veneziana.jpg' },
  { name: 'Polpettone di Carni Bianche e Verdure', src: 'images/dishes/polpettone_carni_bianche_verdure.jpg' },
  { name: 'Merluzzo in Tempura', src: 'images/dishes/merluzzo_in_tempura.jpg' },
  { name: 'Seppie con Piselli', src: 'images/dishes/seppie_con_piselli.jpg' },
  { name: 'Branzino', src: 'images/dishes/branzino.jpg' },
  { name: 'Cotolette di Pollo', src: 'images/dishes/cotolette_di_pollo.jpg' },
  { name: 'Platessa', src: 'images/dishes/platessa.jpg' },
  { name: 'Ricciola', src: 'images/dishes/ricciola.jpg' },
  { name: 'Gallinella con Pomodorini e Olive', src: 'images/dishes/gallinella_pomodorini_olive.jpg' },
  { name: 'Sugo di Pesce', src: 'images/dishes/sugo_di_pesce.jpg' },
  { name: 'Peperonata', src: 'images/dishes/peperonata.jpg', season: 'estate' },
  { name: 'Sarde in Saor', src: 'images/dishes/sarde_in_saor.jpg' },
  { name: 'Lenticchie', src: 'images/dishes/lenticchie.jpg', season: 'inverno' },
  { name: 'Polpette di Verdure', src: 'images/dishes/polpette_di_verdure.jpg' },
  { name: 'Fagioli con Cipolla', src: 'images/dishes/fagioli_con_cipolla.jpg', season: 'estate' },
  { name: 'Fagioli al Prezzemolo', src: 'images/dishes/fagioli_al_prezzemolo.jpg' },
  { name: 'Insalata di Orzo', src: 'images/dishes/insalata_di_orzo.jpg', season: 'estate' },
  { name: 'Crostata di Albicocca', src: 'images/dishes/crostata_di_albicocca.jpg' },
  { name: 'Crostata ai Frutti di Bosco', src: 'images/dishes/crostata_ai_frutti_di_bosco.jpg' },
  { name: 'Frittelle', src: 'images/dishes/frittelle.jpg', season: 'carnevale' },
  { name: 'Crostoli', src: 'images/dishes/crostoli.jpg', season: 'carnevale' },
  { name: 'Ciambella alla Ricotta', src: 'images/dishes/ciambella_alla_ricotta.jpg' },
  { name: 'Selezione di Formaggi', src: 'images/dishes/formaggi_misti.jpg' },
  { name: 'Crema di Pistacchio', src: 'images/dishes/crema_di_pistacchio.jpg', season: 'feste' },
  { name: 'Crema Dolce al Pistacchio', src: 'images/dishes/crema_dolce_al_pistacchio.jpg', season: 'feste' },
  { name: 'Torrone Assortimento', src: 'images/dishes/torrone_assortimento.jpg', season: 'feste' },
  { name: 'Panettone Tradizionale', src: 'images/dishes/panettone_tradizionale.webp', season: 'feste' },
  { name: 'Colomba Classica', src: 'images/dishes/colomba_classica.webp', season: 'pasqua' },
];

/* ─────────────── STAGIONI ───────────────
   Solo alcuni piatti sono legati a una stagione (estate / inverno / carnevale);
   gli altri restano disponibili tutto l'anno e non mostrano etichetta. */
const SEASON_LABEL_VEC = { estate: 'Mesi Caldi', inverno: 'Mesi Fredi', carnevale: 'Carneval', feste: 'Nadal', pasqua: 'Pasqua' };
/* Stagioni "a festa" (Carnevale, Natale, Pasqua): quando una di queste è quella in corso
   (o forzata dal menù a tendina), i piatti di quella stagione vengono evidenziati nel menù
   con bordo, alone pulsante e didascalia nel colore della stagione. */
const FESTIVE_SEASONS = ['carnevale', 'feste', 'pasqua'];
/* #RRGGBB -> rgba(r,g,b,a), per derivare aloni e ombre dal colore della stagione. */
function seasonRgba(hex, a) {
  const n = parseInt(hex.slice(1), 16);
  return `rgba(${(n >> 16) & 255},${(n >> 8) & 255},${n & 255},${a})`;
}
const SEASONS = {
  estate:    { label: 'Mesi Caldi',   labelEn: 'Warm Months',  short: 'Mesi Caldi',  shortEn: 'Warm Months',  shortVec: 'Mesi Caldi',  color: '#D4A640' },
  inverno:   { label: 'Mesi Freddi',  labelEn: 'Cold Months',  short: 'Mesi Freddi', shortEn: 'Cold Months',  shortVec: 'Mesi Fredi',  color: '#5E7488' },
  carnevale: { label: 'Carnevale',    labelEn: 'Carnival',     short: 'Carnevale',   shortEn: 'Carnival',     shortVec: 'Carneval',    color: '#C8251D' },
  feste:     { label: 'Natale',       labelEn: 'Christmas',    short: 'Natale',      shortEn: 'Christmas',    shortVec: 'Nadal',       color: '#2F5D3A' },
  pasqua:    { label: 'Pasqua',       labelEn: 'Easter',       short: 'Pasqua',      shortEn: 'Easter',       shortVec: 'Pasqua',      color: '#8E6FA6' },
};

/* Data di Pasqua (algoritmo gregoriano di Gauss) — serve per calcolare il Martedì Grasso. */
...
/* Martedì Grasso = Pasqua - 47 giorni. */
...
/* Date del prossimo Carnevale (6 gennaio – Martedì Grasso), per l'etichetta accanto
   al titolo "Alcuni Sapori di Carnevale". Se il Carnevale di quest'anno è già
   passato, mostra quello dell'anno successivo. */
...
/* Parametri stagionali (date reali, non solo mesi):
   Estate: 1 maggio – 30 settembre
   Inverno: 1 ottobre – 31 dicembre
   Carnevale: 1 gennaio – Martedì Grasso
   Nessuna sovrapposizione tra le finestre: da fine Carnevale a fine aprile, e in
   nessun altro momento, si mostra "Tutto l'anno". */
...
// Rotazione "per caricamento pagina": un nuovo seme casuale viene generato a
// ogni caricamento (anche un semplice refresh), così i piatti in evidenza
// cambiano ogni volta che la pagina viene ricaricata.
...
function getSeasonForDate(date) {
  ...
  // Feste: 1 dicembre – 6 gennaio (a cavallo dell'anno).
  ...
  // Pasqua: le tre settimane che precedono la Pasqua, domenica di Pasqua inclusa.
  // (Prima erano ~5 settimane e mezza: troppo lunghe per un picco d'acquisto
  // concentrato negli ultimi giorni. Per allungare/accorciare: cambiare il 21.)
  const easter = getEasterDate(year);
  const pasquaStart = new Date(easter);
  pasquaStart.setDate(pasquaStart.getDate() - 21);
  if (d0 >= pasquaStart && d0 <= easter) return 'pasqua';

  // Estate/Inverno del menù seguono lo stesso cambio ora legale (UE) degli orari:
  // l'estate comincia l'ultima domenica di marzo e finisce l'ultima domenica di ottobre;
  // il resto dell'anno (escluso i periodi speciali sopra) è inverno.
  const summerStart = getLastSundayOfMonth(year, 2); // ultima domenica di marzo
  const winterStart = getLastSundayOfMonth(year, 9); // ultima domenica di ottobre
  if (d0 >= summerStart && d0 < winterStart) return 'estate';
  return 'inverno';
}
/* Etichetta "1 dicembre AAAA – 6 gennaio AAAA" per il periodo di Feste in corso o
   il prossimo, usata accanto al titolo "Alcuni Sapori di Feste". */
...
/* Etichetta per il periodo di Pasqua in corso o il prossimo (le tre settimane che
   precedono la Pasqua), usata accanto al titolo "Alcuni Sapori di Pasqua". */
...
/* Click-to-reveal descriptions. Edit freely — add or refine as you go. */
...
const DISH_NAME_EN = {
  'Riso Rosso': 'Red Rice Salad', 'Vitello Tonnato': 'Vitello Tonnato', 'Orecchiette': 'Orecchiette',
  'Pasta Pomodorini Cacio e Pepe': 'Pasta with Cherry Tomatoes, Cacio e Pepe', 'Pomodori Ripieni': 'Stuffed Tomatoes',
  'Insalata Russa': 'Russian Salad', 'Pasta Radicchio & Salsiccia': 'Pasta with Radicchio & Sausage',
  'Pasta Fredda': 'Cold Pasta Salad', 'Insalata di Pasta': 'Pasta Salad', 'Pasta Italia': 'Pasta Italia',
  'Insalata di Riso': 'Rice Salad', 'Insalata di Farro': 'Farro Salad', 'Farro Vegano': 'Vegan Farro Salad',
  'Insalata di Mare': 'Seafood Salad', 'Gnocchi di Zucca': 'Pumpkin Gnocchi', 'Cipolle al Forno': 'Baked Onions',
  'Pollo alla Piastra': 'Grilled Chicken', 'Cappelletti Mostarda': 'Sweet Cappelletti with Mostarda',
  'Porchetta di Nostra Produzione': 'House-Made Porchetta', 'Scaloppine di Pollo al Marsala': 'Chicken Scaloppine in Marsala',
  'Cioccolato Laica': 'Laica Chocolate', 'Pane di Semola Forte': 'Semolina Bread', 'Taralli Danieli': 'Danieli Taralli',
  'Lasagne Carne': 'Meat Lasagna', 'Lasagne con Asparagi': 'Asparagus Lasagna', 'Roast Beef': 'Roast Beef',
  'Spiedi di Gamberone': 'King Prawn Skewers', 'Involtini Valdostani': 'Valdostana Rolls', 'Zucca al Forno': 'Roasted Pumpkin',
  "Fagiolini con l'Occhio e Acciughine": 'Black-Eyed Beans with Anchovies', 'Verza con Pancetta': 'Savoy Cabbage with Pancetta',
  'Trippa alla Parmigiana': 'Tripe alla Parmigiana', 'Involtini di Tacchino': 'Turkey Rolls',
  'Canederli con Speck': 'Speck Dumplings', 'Baccalà alla Vicentina': 'Baccalà alla Vicentina',
  "Sciroppo per l'inverno": 'Winter Syrup', 'Tonno di Carloforte': 'Carloforte Tuna',
  'Babbi Dolcetorta al pistacchio': 'Pistachio Wafer Cake', 'Cioccolato Origine': 'Origin Chocolate',
  'Prosciutto di Parma 24 Mesi': '24-Month Parma Ham', 'Prosciutto di San Daniele Magnum': 'San Daniele Magnum Ham', 'Frutta in Acquavite Prunotto': 'Prunotto Fruit in Spirits', 'Riso Acquerello 8 Anni': '8-Year Aged Acquerello Rice',
  'Pinzimonio di Verdure Brezzo': 'Sweet & Sour Vegetables', 'Specialità di Grano': 'Wheat Specialties',
  'Tiramisù della casa': 'House Tiramisù', 'Zuppa Inglese': 'Zuppa Inglese Custard Trifle', 'Porcini Tagliati Testa Nera': 'Sliced Porcini Mushrooms',
  'Crema Zabaione con Marsala': 'Zabaione Cream with Marsala', 'Aceto Balsamico IGP Campari': 'Balsamic Vinegar IGP — Campari',
  'Aceto Balsamico Oro Gold': 'Balsamic Vinegar — Oro Gold', 'Confettura Extra Mistilli': 'Mixed Berry Jam',
  'Albicocche in Grappa Brezzo': 'Apricots in Grappa', 'Distillati Of Bonollo 1908': 'Bonollo Of Spirits',
  'Fusilli di Pisa Martelli': 'Martelli Fusilli', 'Ragù di Cervo Regi': 'Venison Ragù',
  'Grissini Il Panificio': 'Breadsticks', 'Prosecco Astoria & Fongaro': 'Prosecco Gift Set',
  'Ciambelline Bergamini': 'Bergamini Ciambelline', 'Vini Assortiti': 'Assorted Wines',
  'Melanzane "Pizzaiola"': 'Eggplant "Pizzaiola"', 'Verdure Ripiene': 'Stuffed Vegetables', 'Faraona Arrosto': 'Roast Guinea Fowl',
  'Spezzatino di Tacchino': 'Turkey Stew', 'Fagioli in "Potacin"': 'Beans in "Potacin" Sauce', "Pesto d'Agrumi": 'Citrus Pesto',
  'Riso Venere': 'Venere Black Rice', 'Coniglio in Salmì': 'Rabbit in Salmì', 'Polpettone di Vitello': 'Veal Meatloaf',
  'Tacchino Farcito': 'Stuffed Turkey', 'Arrosto di Vitello al Forno': 'Roast Pork', 'Brasato al Vino': 'Wine-Braised Beef',
  'Pollo con Patate': 'Chicken with Potatoes',
  'Gnocchi alla Romana': 'Gnocchi alla Romana', 'Gnocchi alla Sorrentina': 'Gnocchi alla Sorrentina', 'Pasticciata': 'Pasticciata', 'Stoccafisso': 'Stockfish', 'Fondi di Carciofi': 'Artichoke Hearts',
  'Puré di Patate': 'Mashed Potatoes', 'Patate al Rosmarino al Forno': 'Roasted Rosemary Potatoes',
  'Salmone agli Agrumi': 'Citrus Salmon', 'Capesante': 'Scallops', 'Bistecca di Tonno': 'Tuna Steak', 'Piselli': 'Peas', 'Spinaci': 'Spinach', 'Prodotti Tipici Rovigo': 'Local Rovigo Products', 'Selezione Champagne e Spumanti': 'Champagne & Sparkling Wine Selection', 'Bellavista Alma Grande Cuvée Brut': 'Bellavista Franciacorta Brut',
  'Fegato alla Veneziana': 'Venetian-Style Liver', 'Polpettone di Carni Bianche e Verdure': 'White Meat & Vegetable Meatloaf',
  'Merluzzo in Tempura': 'Cod Tempura', 'Seppie con Piselli': 'Cuttlefish with Peas', 'Branzino': 'Sea Bass',
  'Cotolette di Pollo': 'Chicken Cutlets', 'Platessa': 'Plaice', 'Ricciola': 'Amberjack',
  'Gallinella con Pomodorini e Olive': 'Gurnard with Cherry Tomatoes & Olives', 'Sugo di Pesce': 'Seafood Sauce',
  'Peperonata': 'Peperonata', 'Sarde in Saor': 'Sarde in Saor', 'Lenticchie': 'Lentils',
  'Polpette di Verdure': 'Vegetable Meatballs', 'Fagioli con Cipolla': 'Beans with Onion', 'Fagioli al Prezzemolo': 'Beans with Parsley',
  'Insalata di Orzo': 'Barley Salad', 'Fagioli con Cipolla': 'Beans with Onion', 'Crostata di Albicocca': 'Apricot Tart',
  'Crostata ai Frutti di Bosco': 'Mixed Berry Tart', 'Frittelle': 'Carnival Fritters', 'Crostoli': 'Crostoli',
  'Ciambella alla Ricotta': 'Ricotta Ring Cake', 'Selezione di Formaggi': 'Cheese Selection',
  "Foglie d'Ulivo agli Spinaci": 'Spinach Olive-Leaf Pasta', 'Torronfetta Barbero': 'Barbero Nougat Slices',
  'Marmellata di Limoni di Sicilia': 'Sicilian Lemon Marmalade', 'Confettura di Mirtilli Brezzo': 'Brezzo Blueberry Jam',
  "Miele all'Arancio Brezzo": 'Brezzo Orange Honey', 'Confetture Albicocca e Pesca Lazzaris': 'Lazzaris Apricot & Peach Jams',
  'Gianduiotto Fondente Barbero': 'Barbero Dark Gianduiotto', 'Olio Extra Vergine di Oliva Biologico Salvagno': 'Salvagno Organic Extra Virgin Olive Oil',
  'Crema di Pistacchio': 'Pistachio Pesto', 'Crema Dolce al Pistacchio': 'Sweet Pistachio Spread',
  'Torrone Assortimento': 'Assorted Torrone',
  'Panettone Tradizionale': 'Panettone', 'Colomba Classica': 'Colomba',
};
...
/* Products available in multiple flavors/variants — shows a small badge on the card. */
const DISH_VARIANTS = {};

/* ─────────────────── MENU SECTIONS ───────────────────
   Each section lists dish *names* (must match a name in MENU_DISHES above).
   To add a dish: add a new entry to MENU_DISHES, then list its name here.
   To remove or move a dish between sections, just edit the arrays below.
   Sections can have any number of dishes — they don't need to match.
*/
const MENU_SECTIONS = [
  {
    id: 'antipasti',
    ...
    dishes: [
      ...
      'Peperonata',
      'Zucca al Forno',
      "Fagiolini con l'Occhio e Acciughine",
      'Verza con Pancetta',
      'Puré di Patate',
  ...
  },
  {
    ...
    dishes: [
      ...
      'Polpettone di Carni Bianche e Verdure',
      'Merluzzo in Tempura',
      'Seppie con Piselli',
      'Branzino',
      'Ricciola',
  ...
  },
  {
    ...
    dishes: [
      ...
      'Prodotti Tipici Rovigo',
      "Foglie d'Ulivo agli Spinaci",
      'Olio Extra Vergine di Oliva Biologico Salvagno',
      'Torronfetta Barbero',
      'Gianduiotto Fondente Barbero',
      'Confettura di Mirtilli Brezzo',
      "Miele all'Arancio Brezzo",
      'Albicocche in Grappa Brezzo',
      'Pinzimonio di Verdure Brezzo',
      'Distillati Of Bonollo 1908',
      'Porcini Tagliati Testa Nera',
      'Marmellata di Limoni di Sicilia',
      'Confetture Albicocca e Pesca Lazzaris',
      'Cioccolato Laica',
      'Pane di Semola Forte',
      'Taralli Danieli',
      'Confettura Extra Mistilli',
      'Crema Zabaione con Marsala',
      'Aceto Balsamico IGP Campari',
      'Aceto Balsamico Oro Gold',
      "Sciroppo per l'inverno",
      'Tonno di Carloforte',
      'Babbi Dolcetorta al pistacchio',
      'Cioccolato Origine',
      'Prosciutto di Parma 24 Mesi',
      'Prosciutto di San Daniele Magnum',
      'Frutta in Acquavite Prunotto',
      'Riso Acquerello 8 Anni',
      'Fusilli di Pisa Martelli',
      'Ragù di Cervo Regi',
      'Grissini Il Panificio',
      'Prosecco Astoria & Fongaro',
      'Ciambelline Bergamini',
      'Vini Assortiti',
      'Specialità di Grano',
      'Selezione Champagne e Spumanti',
      'Bellavista Alma Grande Cuvée Brut',
      'Crema di Pistacchio',
      'Crema Dolce al Pistacchio',
      'Torrone Assortimento',
...
function MenuSection() {
  const [selected, setSelected] = useState(null); // { sectionId, name } | null
  const vp = useViewport();
  const lang = useLang().lang;
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(entries => {
      entries.forEach(entry => { if (entry.isIntersecting) setActiveSectionId(entry.target.dataset.sectionId); });
  ...
  const dishByName = React.useMemo(() => {
    const m = {};
    MENU_DISHES.forEach(d => { m[d.name] = d; });
    return m;
  }, []);
  const [autoSeasonKey, setAutoSeasonKey] = useState(() => getSeasonForDate(new Date()));
  // Ricontrolla la data periodicamente così il cambio di stagione (e il Martedì Grasso,
  // che cade in un giorno diverso ogni anno) scatta da solo senza ricaricare la pagina.
  useEffect(() => {
    const check = () => setAutoSeasonKey(getSeasonForDate(new Date()));
    const id = setInterval(check, 60 * 60 * 1000); // ogni ora
  ...
  const featuredSeasonDishes = React.useMemo(
    () => pickRotating(inSeasonDishes, 4, visitSeed),
    [inSeasonDishes, visitSeed]
  );
  return (
    <section style={{ padding: vp.isMobile ? '48px 18px' : '72px 40px', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: vp.isMobile ? 36 : 56 }}>
        <div style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 10 : 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 10 }}>{lang === 'vec' ? 'Par tute le ocasión' : lang === 'en' ? 'For every occasion' : 'Per tutte le occasioni'}</div>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: vp.isPhone ? 26 : vp.isMobile ? 30 : 38, fontWeight: 700, color: '#1A1108', letterSpacing: '-0.01em' }}>{lang === 'vec' ? 'I Nostri Piati' : lang === 'en' ? 'Our Dishes' : 'I Nostri Piatti'}</h2>
        <p style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: vp.isMobile ? 14 : 16, color: '#6B4C33', marginTop: 10 }}>{lang === 'vec' ? 'Toca un piato par védar la ricéta' : lang === 'en' ? 'Tap a dish to reveal the recipe' : 'Tocca un piatto per scoprire la ricetta'}</p>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 12.5 : 13.5, color: '#8A7A63', marginTop: 12, maxWidth: 560, marginLeft: 'auto', marginRight: 'auto', lineHeight: 1.6 }}>{lang === 'vec' ? "El menù el xe tuto quel che savémo far: ma cosa che ghe xe pronto el varia de dì in dì, secondo la stagión e cosa che ghe pias al cogo quela matina — se rivè tardi, magari el xe za finìo!" : lang === 'en' ? "The menu represents our full range: availability of individual ready-made dishes may vary day to day, according to season and ingredient freshness." : "Il menù rappresenta l'intera nostra proposta: la disponibilità dei singoli piatti pronti può variare di giorno in giorno, secondo stagione e freschezza degli ingredienti."}</p>
      </div>

      {/* Sticky section nav / scroll-spy (revert: see MENU-PAGE-ENHANCEMENTS-NOTES.md) */}
      <div style={{
        position: 'sticky', top: vp.isMobile ? 60 : 72, zIndex: 40,
        background: 'rgba(253,250,244,0.96)', backdropFilter: 'blur(8px)',
        ...
        {MENU_SECTIONS.map(section => (
          <a key={section.id} href={`#section-${section.id}`} onClick={e => { e.preventDefault(); document.getElementById(`section-${section.id}`)?.scrollIntoView({ behavior: 'smooth' }); }} style={{
            flex: '0 0 auto', fontFamily: 'var(--font-body)', fontSize: 12.5, fontWeight: 600, letterSpacing: '0.02em',
            padding: '7px 14px', borderRadius: 999, textDecoration: 'none', whiteSpace: 'nowrap',
            color: activeSectionId === section.id ? '#FDFAF4' : '#3D2B1A',
            background: activeSectionId === section.id ? '#C8251D' : 'rgba(26,17,8,0.06)',
            transition: 'all 0.25s ease',
          }}>{lang === 'en' ? section.titleEn : lang === 'vec' ? (section.titleVec || section.title) : section.title}</a>
        ))}
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 14, marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', opacity: 0.85 }}>
              {lang === 'en' ? 'In season now' : lang === 'vec' ? 'Qualcosa in stagion adesso' : 'Alcuni in stagione adesso'}
            </div>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: vp.isMobile ? 20 : 26, fontWeight: 700, margin: '4px 0 0', display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', gap: 10 }}>
              <span>{lang === 'vec' ? `Qualche Savor ${(curSeasonKey === 'estate' || curSeasonKey === 'inverno') ? 'par i' : 'de'} ${curSeason.shortVec}` : lang === 'en' ? `Some Flavors ${(curSeasonKey === 'estate' || curSeasonKey === 'inverno') ? 'for the' : 'of'} ${curSeason.labelEn}` : `Alcuni Sapori ${(curSeasonKey === 'estate' || curSeasonKey === 'inverno') ? 'per i' : 'di'} ${curSeason.label}`}</span>
              {curSeasonKey === 'carnevale' && (
                <span style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 12 : 14, fontWeight: 600, opacity: 0.85 }}>{getUpcomingCarnevaleRangeLabel(new Date(), lang)}</span>
              )}
              {(curSeasonKey === 'estate' || curSeasonKey === 'inverno') && (
                <span style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 12 : 14, fontWeight: 600, opacity: 0.85 }}>{getUpcomingDstRangeLabel(curSeasonKey, new Date(), lang)}</span>
              )}
              {curSeasonKey === 'feste' && (
                <span style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 12 : 14, fontWeight: 600, opacity: 0.85 }}>{getUpcomingFesteRangeLabel(new Date(), lang)}</span>
              )}
              {curSeasonKey === 'pasqua' && (
                <span style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 12 : 14, fontWeight: 600, opacity: 0.85 }}>{getUpcomingPasquaRangeLabel(new Date(), lang)}</span>
              )}
            </h3>
          </div>
        </div>
        {featuredSeasonDishes.length ? (
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap, justifyItems: 'center', marginLeft: vp.isMobile ? -18 : -40, marginRight: vp.isMobile ? -18 : -40, paddingLeft: 14, paddingRight: 14 }}>
            {featuredSeasonDishes.map(d => (
              <div key={d.name} style={{ width: '100%', textAlign: 'center' }}>
                <div style={{ width: '100%', aspectRatio: '1/1', borderRadius: '50%', overflow: 'hidden', border: '3px solid #FDFAF4', boxShadow: '0 6px 16px rgba(26,17,8,0.25)' }}>
                  <img src={d.src} alt={d.name} loading="lazy" decoding="async" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: DISH_POSITIONS[d.name] || 'center center' }} />
                </div>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: vp.isMobile ? 11 : 12, fontWeight: 600, marginTop: 8, lineHeight: 1.25 }}>{lang === 'en' ? (DISH_NAME_EN[d.name] || d.name) : lang === 'vec' ? (DISH_NAME_VEC[d.name] || d.name) : d.name}</div>
              </div>
            ))}
          </div>
        ) : (
          <div style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 13, opacity: 0.9 }}>{lang === 'vec' ? "Deso a brilar xe i nostri clàsici de tuto l'ano." : lang === 'en' ? 'Right now our year-round classics take center stage.' : "In questo periodo brillano i nostri classici di tutto l'anno."}</div>
        )}
      </div>

      {MENU_SECTIONS.map((section, sIdx) => {
        const END_SEASON_ORDER = { carnevale: 1, feste: 2, pasqua: 3 };
        const curDstSeason = getHoursSeasonForDate(new Date());
        const sectionDishes = section.dishes
          .map(name => dishByName[name])
          .filter(Boolean)
          .map((d, i) => ({ d, i }))
          .sort((a, b) => {
          ...
          .map(x => x.d);
        return (
          <MenuSectionReveal key={section.id} sectionId={section.id} sectionRefs={sectionRefs} style={{ marginBottom: vp.isMobile ? 48 : 72 }}>
            {/* Section header */}
            <div style={{
              display: 'flex',
              alignItems: 'center',
              ...
              <div style={{ flex: '0 0 auto', display: 'flex', alignItems: 'baseline', gap: 12 }}>
                ...
                }}>{String(sIdx + 1).padStart(2, '0')}</div>
                <h3 style={{
                  fontFamily: 'var(--font-display)',
                  ...
                  margin: 0,
                }}>{lang === 'en' ? section.titleEn : lang === 'vec' ? (section.titleVec || section.title) : section.title}</h3>
              </div>
              <div style={{ flex: 1, height: 1, background: 'linear-gradient(to right, rgba(26,17,8,0.18), rgba(26,17,8,0))' }} />
              {section.subtitle && (
                <div style={{
                  fontFamily: 'var(--font-display)',
                  fontStyle: 'italic',
                ...
                }}>{section.subtitle && (lang === 'en' ? section.subtitleEn : lang === 'vec' ? (DISH_SUBTITLE_VEC[section.id] || section.subtitle) : section.subtitle)}</div>
              )}
            </div>

            {/* Empty-state placeholder so adding products feels obvious */}
            {sectionDishes.length === 0 ? (
              <div style={{
                ...
                {section.id === 'prodotti'
                  ? (lang === 'en' ? 'No products on special right now — come back and see us.' : "Nessun prodotto in promozione al momento — torna a trovarci.")
                  : (lang === 'en' ? 'Coming soon.' : "In arrivo presto.")}
              </div>
            ) : (
              <div style={{
                display: 'grid',
                ...
                {sectionDishes.map((dish) => {
                  ...
                  return (
                    <figure
                      ...
                      tabIndex={0}
                      aria-expanded={isSel}
                      onClick={() => setSelected(isSel ? null : { sectionId: section.id, name: dish.name })}
                      onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setSelected(isSel ? null : { sectionId: section.id, name: dish.name }); } }}
                      style={{ margin: 0, width: '100%', textAlign: 'center', cursor: 'pointer', outline: 'none' }}
                    >
                      <div style={{
                        position: 'relative',
                        width: '100%',
                        ...
                        {dish.season && (
                          <span style={{
                            ...
                            whiteSpace: 'nowrap',
                          }}>{lang === 'en' ? SEASONS[dish.season].shortEn : lang === 'vec' ? SEASONS[dish.season].shortVec : SEASONS[dish.season].short}</span>
                        )}
                        <div style={{ width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden', background: '#FDFAF4' }}>
                          <img
                            src={dish.src}
                            alt={dish.name}
                            loading="lazy"
                        ...
                        {variants && (
                          <div style={{
                            ...
                            whiteSpace: 'nowrap',
                          }}>+{variants.length} {lang === 'en' ? 'flavors' : 'gusti'}</div>
                        )}
                      </div>
                      <figcaption style={{
                        fontFamily: 'var(--font-display)',
      ...
      <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} aria-label="Torna in cima" style={{
        position: 'fixed', right: vp.isMobile ? 14 : 20, bottom: vp.isMobile ? 14 : 20, zIndex: 200,
        width: 44, height: 44, borderRadius: '50%', background: '#1A1108', color: '#FDFAF4', border: 'none',
        boxShadow: '0 4px 16px rgba(26,17,8,0.30)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
        opacity: showTop ? 1 : 0, pointerEvents: showTop ? 'auto' : 'none', transition: 'opacity 0.3s ease',
      }}>
...
/* Crediti dei brani del player. Le due voci CC BY-SA richiedono l'attribuzione;
   le altre sono di pubblico dominio (credito per correttezza, non per obbligo).

   ── FONTE DEL FILE (per rispondere a un'eventuale segnalazione di copyright) ──
   Inno d'Italia ... U.S. Navy Band — PD (opera del governo USA) */
const MUSIC_CREDITS = [
  { title: "Inno d'Italia", detail: 'U.S. Navy Band', license: 'pubblico dominio' },
];
/* ─────────────────── FOOTER ─────────────────── */
function Footer({ onNavigate }) {
  ...
  const navLabels = lang === 'vec'
    ? { home: 'Casa', menu: 'El Menù', ricette: 'Ricéte', about: 'Chi Sémo', contact: 'Contàti' }
    : lang === 'en'
    ? { home: 'Home', menu: 'Menu', ricette: 'Recipes', about: 'About Us', contact: 'Contact' }
    : { home: 'Home', menu: 'Il Menù', ricette: 'Ricette', about: 'Chi Siamo', contact: 'Contatti' };
  return (
    <footer style={{ background: '#1A1108', color: '#E8DCC8', padding: vp.isMobile ? '40px 20px 24px' : '56px 40px 32px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <img src={window.IMGS.logo} alt="F.lli Piva" loading="lazy" decoding="async" style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', opacity: 0.92 }} />
              <div>
                <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, color: '#FDFAF4' }}>F.lli Piva</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {Object.entries(navLabels).map(([id, label]) => (
                <button key={id} onClick={() => onNavigate(id)} style={{
                  ...
                  fontFamily: 'var(--font-body)', fontSize: 13, color: '#C9A97A', padding: 0, transition: 'color 0.2s',
                }}
          ...
          <div>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#D4A640', marginBottom: 16 }}>{lang === 'vec' ? 'Ndo che sémo' : lang === 'en' ? 'Find Us' : 'Dove Siamo'}</div>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#C9A97A', lineHeight: 2 }}>
              <div>Piazza G. Garibaldi, 15</div>
              <div>45100 Rovigo (RO)</div>
              <div style={{ marginTop: 8 }}>{lang === 'en' ? 'Tel. 0425 24845' : 'Tel. 0425 24845'}</div>
              <div>Facebook: Gastronomia Fratelli PIVA</div>
              {/* Link-out alla radio locale (nessun re-streaming: si apre il player di Delta Radio) */}
              <div style={{ marginTop: 8 }}>
                <a href="https://www.deltaradio.it/sezioni/285/diretta" target="_blank" rel="noopener noreferrer" style={{ color: '#D4A640', textDecoration: 'none', borderBottom: '1px solid rgba(212,166,64,0.35)' }}>
                  {lang === 'vec' ? 'Scoltémo Delta Radio' : lang === 'en' ? 'We listen to Delta Radio' : 'Ascoltiamo Delta Radio'}
                </a>
              </div>
            </div>
          </div>
        ...
        <details style={{ marginTop: 20 }}>
          <summary style={{ fontFamily: 'var(--font-body)', fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#6B4C33', cursor: 'pointer', textAlign: vp.isPhone ? 'left' : 'center' }}>{lang === 'vec' ? 'Crediti de la musica' : lang === 'en' ? 'Music credits' : 'Crediti musicali'}</summary>
          <div style={{ marginTop: 14, display: 'grid', gap: 10, maxWidth: 760, marginLeft: 'auto', marginRight: 'auto' }}>
            {MUSIC_CREDITS.map(c => (
              <div key={c.title} style={{ fontFamily: 'var(--font-body)', fontSize: 10.5, color: '#6B4C33', lineHeight: 1.6 }}>
                <span style={{ color: '#C9A97A' }}>{c.title}</span>{c.detail ? ' — ' + c.detail : ''} · {c.licenseUrl
                  ? <a href={c.licenseUrl} target="_blank" rel="noopener noreferrer" style={{ color: '#6B4C33' }}>{c.license}</a>
                  : c.license}
              </div>
            ))}
          </div>
        </details>
        <div style={{ textAlign: vp.isPhone ? 'left' : 'center', marginTop: 18 }}>
          <a href="mailto:djrdjr@live.com?subject=WEBSITE" style={{ fontFamily: 'var(--font-body)', fontSize: 10, color: '#C9A97A', textDecoration: 'underline', textUnderlineOffset: 2 }}>{lang === 'vec' ? 'Sito fato da' : lang === 'en' ? 'Site by' : 'Sito a cura di'} Daniel John Rainey · djrdjr@live.com</a>
        </div>
      </div>
    </footer>
  );
...
/* ─────────────────── RICETTA DELLA SETTIMANA ───────────────────
   Ricette d'esempio: testo e passaggi sono contenuti segnaposto,
   pensati per essere sostituiti dopo la revisione del cliente.
   Rotazione: una ricetta a settimana, scelta tra quelle della stagione
   in corso (stesso calendario stagionale del menù); fuori stagione
   (fine Carnevale – aprile) si pesca dal pool "tutto l'anno". */
const RECIPES = [
  {
    id: 'farro-vegano', name: 'Farro Vegano', nameEn: "Vegan Farro Salad", season: 'estate',
  ...
  {
    id: 'peperonata', name: 'Peperonata', nameEn: "Peperonata", season: 'estate',
  ...
  {
    id: 'gnocchi-di-zucca', name: 'Gnocchi di Zucca', nameEn: "Pumpkin Gnocchi", season: 'inverno',
  ...
  {
    id: 'baccala-alla-vicentina', name: 'Baccalà alla Vicentina', nameEn: "Baccalà alla Vicentina", season: 'inverno',
  ...
  {
    id: 'frittelle', name: 'Frittelle', nameEn: "Carnival Fritters", season: 'carnevale',
  ...
  {
    id: 'crostoli', name: 'Crostoli', nameEn: "Crostoli", season: 'carnevale',
  ...
  {
    id: 'polpette-di-verdure', name: 'Polpette di Verdure', nameEn: "Vegetable Meatballs", season: null,
  ...
  {
    id: 'insalata-di-farro', name: 'Insalata di Farro', nameEn: "Farro Salad", season: null,
  ...
  {
    id: 'insalata-di-riso', name: 'Insalata di Riso', nameEn: "Rice Salad", season: 'estate',
  ...
  {
    id: 'riso-venere', name: 'Riso Venere', nameEn: "Venere Black Rice", season: 'estate',
  ...
  {
    id: 'canederli-con-speck', name: 'Canederli con Speck', nameEn: "Speck Dumplings", season: 'inverno',
  ...
  {
    id: 'zucca-al-forno', name: 'Zucca al Forno', nameEn: "Roasted Pumpkin", season: 'inverno',
  ...
  {
    id: 'faraona-arrosto', name: 'Faraona Arrosto', nameEn: "Roast Guinea Fowl", season: 'inverno',
  ...
  {
    id: 'lenticchie', name: 'Lenticchie', nameEn: "Lentils", season: 'inverno',
...
/* Sceglie la ricetta della settimana tra: le ricette senza tag di stagione
   (disponibili tutto l'anno) e quelle della stagione in corso. La scelta è
   deterministica in base al numero di settimana ISO, così resta la stessa per
   tutti i visitatori per sette giorni e cambia da sola ogni lunedì. */
const RECIPE_VEC = {
  'farro-vegano': {
...
/* Numero di settimana ISO 8601 (settimana che inizia il lunedì). */
...
function RecipePage() {
  ...
  return (
    <section style={{ maxWidth: 900, margin: '0 auto', padding: vp.isMobile ? '32px 18px 60px' : '56px 40px 90px' }}>
      <div style={{ borderRadius: 14, overflow: 'hidden', boxShadow: '0 4px 24px rgba(26,17,8,0.14)', background: '#F5F0E6' }}>
        <div style={{ position: 'relative', height: vp.isMobile ? 220 : 360 }}>
          <img src={recipe.img} alt={lang === 'en' ? recipe.nameEn : recipe.name} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(26,17,8,0.75), transparent 60%)' }} />
          <div style={{ position: 'absolute', left: vp.isMobile ? 20 : 40, bottom: vp.isMobile ? 16 : 28, color: '#FDFAF4' }}>
            {seasonInfo && (
              <span style={{ display: 'inline-block', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#fff', background: seasonInfo.color, padding: '4px 12px', borderRadius: 999 }}>{lang === 'en' ? seasonInfo.labelEn : lang === 'vec' ? (SEASON_LABEL_VEC[recipe.season] || seasonInfo.label) : seasonInfo.label}</span>
            )}
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: vp.isMobile ? 26 : 40, margin: '10px 0 0' }}>{lang === 'en' ? recipe.nameEn : lang === 'vec' ? (DISH_NAME_VEC[recipe.name] || recipe.name) : recipe.name}</h1>
          </div>
        </div>
        <div style={{ padding: vp.isMobile ? '24px 20px' : '36px 44px' }}>
          <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#C8251D' }}>{lang === 'vec' ? 'Ricéta de la stimana' : lang === 'en' ? 'Recipe of the Week' : 'Ricetta della settimana'}</div>
          <p style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 17, color: '#6B4C33', margin: '10px 0 30px' }}>{lang === 'en' ? recipe.taglineEn : lang === 'vec' ? ((RECIPE_VEC[recipe.id]||{}).tagline || recipe.tagline) : recipe.tagline}</p>
          <div style={{ display: 'grid', gridTemplateColumns: vp.isMobile ? '1fr' : '1fr 1.4fr', gap: vp.isMobile ? 28 : 40 }}>
            <div>
              <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 18, margin: '0 0 14px', color: '#1A1108' }}>{lang === 'vec' ? 'Ingredienti' : lang === 'en' ? 'Ingredients' : 'Ingredienti'}</h3>
              <ul style={{ margin: 0, paddingLeft: 18, lineHeight: 2, fontSize: 14, color: '#3D2B1A' }}>
                {(lang === 'en' ? recipe.ingredientsEn : lang === 'vec' ? ((RECIPE_VEC[recipe.id]||{}).ingredients || recipe.ingredients) : recipe.ingredients).map((ing, i) => <li key={i}>{ing}</li>)}
              </ul>
            </div>
            <div>
              <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 18, margin: '0 0 14px', color: '#1A1108' }}>{lang === 'vec' ? 'Come se fa' : lang === 'en' ? 'Method' : 'Procedimento'}</h3>
              <ol style={{ margin: 0, paddingLeft: 18, lineHeight: 1.9, fontSize: 14, color: '#3D2B1A' }}>
                {(lang === 'en' ? recipe.stepsEn : lang === 'vec' ? ((RECIPE_VEC[recipe.id]||{}).steps || recipe.steps) : recipe.steps).map((s, i) => <li key={i}>{s}</li>)}
              </ol>
            </div>
          </div>
        </div>
...
function AboutPage() {
  ...
  const en = lang === 'en';
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: vp.isMobile ? '48px 20px' : '80px 40px' }}>
      ...
      <ScrollReveal style={{ display: 'grid', gridTemplateColumns: vp.isMobile ? '1fr' : '1fr 1fr', gap: vp.isMobile ? 28 : 40, alignItems: 'start', marginBottom: vp.isMobile ? 40 : 56 }}>
        <div>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 14 : 15, color: '#3D2B1A', lineHeight: 1.85 }}>
            {lang === 'vec' ? <React.Fragment>Incuò i Fradèi Piva i tien in man ogni roba de persona: da la salumeria a la gastronomia fata ogni dì, dai vini e formai fin ai pachéti regalo e le ceste de Nadal. <em>Granda profesionalità e tanta cortesia</em>, sempre.</React.Fragment> : en ? <React.Fragment>Today Fratelli Piva runs every aspect directly: from the delicatessen to the prepared foods made fresh each day, from the selection of wines and cheeses to gift packages and holiday baskets. <em>Great professionalism and kindness</em>, always.</React.Fragment> : <React.Fragment>Oggi Fratelli Piva gestisce direttamente ogni aspetto: dalla salumeria alla gastronomia preparata ogni giorno, dalla selezione di vini e formaggi alle confezioni regalo e ceste natalizie. <em>Grande professionalità e gentilezza</em>, sempre.</React.Fragment>}
          </p>
        </div>
        <div>
          <img src={window.IMGS.storefront} alt="Gastronomia F.lli Piva — Piazza Garibaldi, Rovigo" loading="lazy" decoding="async" style={{ width: '100%', height: vp.isMobile ? 220 : 300, objectFit: 'cover', borderRadius: 14, boxShadow: '0 4px 20px rgba(26,17,8,0.14)', display: 'block' }} />
          <div style={{ fontFamily: 'var(--font-body)', fontStyle: 'italic', fontSize: 11, color: '#6B4C33', marginTop: 8, textAlign: 'center' }}>Piazza G. Garibaldi, 15 · Rovigo</div>
        </div>
      </ScrollReveal>

      {/* Pull quote */}
      <ScrollReveal>
      <blockquote style={{ borderLeft: '4px solid #C8251D', background: '#FDFAF4', padding: vp.isMobile ? '20px 22px' : '24px 28px', borderRadius: '0 12px 12px 0', boxShadow: '0 2px 8px rgba(26,17,8,0.07)', marginBottom: vp.isMobile ? 40 : 56 }}>
        <p style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: vp.isMobile ? 17 : 21, color: '#1A1108', lineHeight: 1.5, marginBottom: 10 }}>{en ? '"A paradise for food lovers — great professionalism and kindness, with the care of those who love their work."' : '"Paradiso dei buongustai — grande professionalità e gentilezza, con la cura di chi ama il proprio lavoro."'}</p>
        <cite style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: '#6B4C33', letterSpacing: '0.06em' }}>{en ? '— Il Gazzettino, September 28, 1999' : '— Il Gazzettino, 28 settembre 1999'}</cite>
      </blockquote>
      </ScrollReveal>
      ...
      {/* Gazzettino + team */}
      <ScrollReveal style={{ display: 'grid', gridTemplateColumns: vp.isMobile ? '1fr' : '1.2fr 1fr', gap: vp.isMobile ? 28 : 32, alignItems: 'start', marginBottom: vp.isMobile ? 40 : 56 }}>
        <div>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 10 }}>{lang === 'vec' ? 'Come che i giornai i gà contà de nualtri' : en ? 'As the press told it' : 'Come ci ha raccontato la stampa'}</div>
          <img src={window.IMGS.gazzettino} alt="Il Gazzettino 1999 — Fratelli Piva" loading="lazy" decoding="async" style={{ width: '100%', borderRadius: 10, boxShadow: '0 4px 20px rgba(26,17,8,0.16)', display: 'block' }} />
          <div style={{ fontFamily: 'var(--font-body)', fontStyle: 'italic', fontSize: 11, color: '#6B4C33', marginTop: 8 }}>{en ? 'Il Gazzettino · September 28, 1999 · "Fratelli Piva: showcasing the quality of times past"' : 'Il Gazzettino · 28 settembre 1999 · "Fratelli Piva: in primo piano la qualità del tempo antico"'}</div>
        </div>
        <div>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 10 }}>{lang === 'vec' ? 'La Nostra Squadra' : en ? 'Our Team' : 'Il nostro team'}</div>
          <img src={window.IMGS.team} alt="Il team di F.lli Piva" loading="lazy" decoding="async" style={{ width: '100%', height: 'auto', aspectRatio: '894 / 662', objectFit: 'contain', background: '#F5F0E6', borderRadius: 10, boxShadow: '0 4px 20px rgba(26,17,8,0.14)', display: 'block', marginBottom: 16 }} />
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: '#3D2B1A', lineHeight: 1.8 }}>{lang === 'vec' ? "El nostro personal el se dà da far ogni benedéto dì par darghe a la bòna clientela roba de prima, fata co' ingredienti freschi sielti tute le matine bonora." : en ? 'Our staff works every day with dedication to offer our valued customers top-quality products, prepared with fresh ingredients selected every morning.' : 'Il nostro staff lavora ogni giorno con dedizione per offrire alla gentile clientela prodotti di prima qualità, preparati con ingredienti freschi selezionati ogni mattina.'}</p>
        </div>
      </ScrollReveal>

      {/* Menu document */}
      <ScrollReveal>
      <div style={{ background: '#FDFAF4', borderRadius: 16, padding: vp.isMobile ? '24px 22px' : '32px 36px', boxShadow: '0 2px 12px rgba(26,17,8,0.08)', display: 'grid', gridTemplateColumns: vp.isMobile ? '1fr' : '1fr 1.1fr', gap: vp.isMobile ? 24 : 32, alignItems: 'center' }}>
        <div>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 12 }}>{lang === 'vec' ? 'El Nostro Menù' : en ? 'Our Menu' : 'Il nostro menù'}</div>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: vp.isMobile ? 22 : 26, fontWeight: 700, color: '#1A1108', marginBottom: 14 }}>{lang === 'vec' ? 'Sempre freschi' : en ? 'Always fresh, always home-made' : 'Sempre freschi'}</h2>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: '#3D2B1A', lineHeight: 1.85, marginBottom: 20 }}>{lang === 'vec' ? 'Antipasti, primi, secondi, pese, contorni e dolci — fati freschi ogni santo dì.' : en ? 'Starters, first courses, mains, fish, sides and desserts — prepared fresh every day.' : 'Antipasti, primi, secondi, pesce, contorni e dolci — preparati ogni giorno.'}</p>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: '#6B4C33', fontStyle: 'italic' }}>{en ? 'Tel. and fax 0425 24845' : 'Tel. e fax 0425 24845'}</div>
        </div>
        <img src={window.IMGS.menuDoc} alt="Menù Gastronomia F.lli Piva" loading="lazy" decoding="async" style={{ width: '100%', borderRadius: 10, boxShadow: '0 4px 16px rgba(26,17,8,0.14)', display: 'block' }} />
      </div>
      </ScrollReveal>
...
function HoursTester() {
  ...
  const seasonLabels = lang === 'vec' ? { estate: 'Istà', inverno: 'Inverno', december: 'Dicembre' } : en ? { estate: 'Warm', inverno: 'Cold', december: 'December' } : { estate: 'Estate', inverno: 'Inverno', december: 'Dicembre' };
  ...
  const selectStyle = {
    fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 600, color: '#1A1108',
    background: '#fff', border: '1.5px solid rgba(26,17,8,0.15)', borderRadius: 8,
    padding: '9px 10px', cursor: 'pointer',
  };

  return (
    <div style={{ marginTop: vp.isMobile ? 20 : 28, background: '#FDFAF4', borderRadius: 16, padding: vp.isMobile ? '24px 22px' : '28px 32px', boxShadow: '0 2px 12px rgba(26,17,8,0.08)', border: '1px solid rgba(200,37,29,0.20)' }}>
      <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 10 }}>{lang === 'vec' ? 'Par i nostri clienti' : en ? 'For our customers' : 'Per i nostri clienti'}</div>
      <div style={{ fontFamily: 'var(--font-display)', fontSize: vp.isMobile ? 18 : 20, fontWeight: 600, color: '#1A1108', marginBottom: 8 }}>{lang === 'vec' ? "Varda se sémo verti in t'un dì che gà da vegner" : en ? 'Check if we are open on a future day' : 'Verifica se siamo aperti in un giorno futuro'}</div>
      <div style={{ fontFamily: 'var(--font-body)', fontSize: 13.5, color: '#6B4C33', marginBottom: 18, lineHeight: 1.5 }}>{lang === 'vec' ? "Sielzi 'na data par saver sùbito se la gastronomia la xe verta e a che ora, feste e cambio de stagión comprese." : en ? "Pick a date to instantly find out if the gastronomia is open and at what time, including holidays and seasonal changes." : 'Scegli una data per sapere subito se la gastronomia è aperta e a che ora, comprese festività e cambio di stagione.'}</div>

      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 18, alignItems: 'center' }}>
        <select value={safeDay} onChange={e => setDay(Number(e.target.value))} style={selectStyle}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: vp.isMobile ? 20 : 40, alignItems: 'baseline' }}>
        <div>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8A7A63', marginBottom: 4 }}>{lang === 'vec' ? 'Dì' : en ? 'Day' : 'Giorno'}</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 600, color: '#1A1108' }}>{dowLabel}</div>
        </div>
        {info.seasonKey && (
          <div>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8A7A63', marginBottom: 4 }}>{lang === 'vec' ? 'Stagión orario' : en ? 'Hours Season' : 'Stagione orario'}</div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 600, color: '#1A1108' }}>{seasonLabel}</div>
          </div>
        )}
        <div>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8A7A63', marginBottom: 4 }}>{lang === 'vec' ? 'Orario calcolà' : en ? 'Calculated Hours' : 'Orario calcolato'}</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 600, color: resultColor }}>{resultLabel}</div>
        </div>
      </div>
    </div>
  );
...
function ContactPage() {
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: vp.isMobile ? '48px 20px' : '80px 40px' }}>
      <MountReveal index={0}>
      <div style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 10 : 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 12 }}>{lang === 'vec' ? 'Sémo qua par voaltri' : en ? 'We are here for you' : 'Siamo a tua disposizione'}</div>
      <h1 style={{ fontFamily: 'var(--font-display)', fontSize: vp.isPhone ? 28 : vp.isMobile ? 32 : 40, fontWeight: 700, color: '#1A1108', marginBottom: 12, lineHeight: 1.15 }}>{lang === 'vec' ? 'Vien a catarne' : en ? 'Come Visit Us' : 'Vieni a trovarci'}</h1>
      <p style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: vp.isMobile ? 14 : 15, color: '#6B4C33', marginBottom: vp.isMobile ? 28 : 40, maxWidth: 600 }}>{lang === 'vec' ? "Par órdini, prenotazion e ceste regalo, ciàmane o fà un salto in botéga: sarémo contenti de darve el benvegnù de persona." : en ? 'For orders, reservations and gift baskets, call us or stop by the shop: we\'ll be happy to welcome you in person.' : 'Per ordini, prenotazioni e ceste regalo, chiamaci o passa in negozio: saremo felici di accoglierti di persona.'}</p>
      </MountReveal>

      <div style={{ display: 'grid', gridTemplateColumns: vp.isMobile ? '1fr' : '1.05fr 1fr', gap: vp.isMobile ? 28 : 40, alignItems: 'start' }}>

        {/* LEFT — Phone CTA + Address */}
        <MountReveal index={1} style={{ display: 'flex', flexDirection: 'column', gap: vp.isMobile ? 20 : 24 }}>
          ...
          <div style={{
            ...
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(253,250,244,0.78)', marginBottom: 12 }}>{lang === 'vec' ? 'Ciàmane' : en ? 'Call Us' : 'Chiamaci'}</div>
            <a href="tel:+39042524845" style={{
              fontFamily: 'var(--font-display)',
          ...
          <div style={{
            ...
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 10 }}>{lang === 'vec' ? 'Ndo che sémo' : en ? 'Find Us' : 'Dove siamo'}</div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: vp.isMobile ? 19 : 22, fontWeight: 600, color: '#1A1108', marginBottom: 4, letterSpacing: '-0.01em' }}>Piazza Giuseppe Garibaldi, 15</div>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: '#6B4C33', marginBottom: 18 }}>45100 Rovigo (RO) · {en ? 'Italy' : 'Italia'}</div>
            <a href={mapsUrl} target="_blank" rel="noopener noreferrer" style={{
              display: 'inline-flex', alignItems: 'center', gap: 8,
              background: 'transparent', color: '#C8251D',
              fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600,
              padding: '9px 18px', borderRadius: 999, border: '1.5px solid #C8251D',
            ...
            >
              ...
              {lang === 'vec' ? 'Verzi su Google Maps' : en ? 'Open in Google Maps' : 'Apri in Google Maps'}
            </a>
          </div>

          {/* Social */}
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#6B4C33', lineHeight: 1.7, paddingTop: 4 }}>
            <span style={{ fontWeight: 600, color: '#3D2B1A' }}>{lang === 'vec' ? 'Seguìne su Facebook:' : en ? 'Follow us on Facebook:' : 'Seguici su Facebook:'}</span> Gastronomia Fratelli PIVA
          </div>
        </MountReveal>

        {/* RIGHT — Hours grid */}
        <MountReveal index={2} style={{}}>
        <div id="hours-panel" style={{
          background: panelBg,
          color: '#E8DCC8',
          borderRadius: 16,
          padding: vp.isMobile ? '28px 24px' : '36px 32px',
          ...
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#D4A640', marginBottom: 14 }}>{lang === 'vec' ? 'Orari de avertura' : en ? 'Opening Hours' : 'Orari di apertura'}</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: vp.isMobile ? 22 : 26, fontWeight: 600, color: '#FDFAF4', marginBottom: 20, letterSpacing: '-0.01em' }}>{lang === 'vec' ? 'Quando che sémo verti' : en ? 'When We\'re Open' : 'Quando siamo aperti'}</div>

          {/* Season tabs */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8, background: 'rgba(232,220,200,0.08)', padding: 4, borderRadius: 999 }}>
            {(lang === 'vec' ? [{ id: 'standard', label: 'Istà' }, { id: 'inverno', label: 'Inverno' }, { id: 'holidays', label: 'Feste' }] : en ? [{ id: 'standard', label: 'Summer' }, { id: 'inverno', label: 'Winter' }, { id: 'holidays', label: 'Holidays' }] : [{ id: 'standard', label: 'Estate' }, { id: 'inverno', label: 'Inverno' }, { id: 'holidays', label: 'Festività' }]).map(({ id, label }) => {
              const active = season === id;
              return (
                <button key={id} onClick={() => setSeason(id)} style={{
                  flex: '1 1 auto',
          ...
          {isHolidays && (
            <div style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: vp.isMobile ? 12.5 : 13, color: '#C9A97A', margin: '10px 0 4px', lineHeight: 1.5 }}>
              {lang === 'vec' ? 'In sti dì de festa sémo serài.' : en ? 'We are closed on the following holidays.' : 'Nei seguenti giorni festivi restiamo chiusi.'}
            </div>
          )}
          {(season === 'standard' || isInverno) && (
            <div style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 11.5 : 12, color: '#C9A97A', margin: '10px 0 4px', lineHeight: 1.5 }}>
              {lang === 'vec' ? `Orario ${season === 'standard' ? 'de l\'istà' : 'de l\'inverno'} — ${getDstRangeLabel(new Date().getFullYear(), season === 'standard' ? 'estate' : 'inverno')}` : en ? `${season === 'standard' ? 'Summer' : 'Winter'} hours — ${getDstRangeLabel(new Date().getFullYear(), season === 'standard' ? 'estate' : 'inverno', 'en')}` : `Orario ${season === 'standard' ? 'estivo' : 'invernale'} — ${getDstRangeLabel(new Date().getFullYear(), season === 'standard' ? 'estate' : 'inverno')}`}
            </div>
          )}

          {isHolidays ? (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {holidays.map(({ name, date }, i) => (
                <div key={name} style={{
                  display: 'flex',
                  justifyContent: 'space-between',
                  ...
                  <div style={{
                    ...
                    color: '#FDFAF4',
                    letterSpacing: '0.01em',
                  }}>{lang === 'vec' ? (HOLIDAY_NAME_VEC[name] || name) : en ? (HOLIDAY_NAME_EN[name] || name) : name}</div>
                  <div style={{
                    fontFamily: 'var(--font-body)',
                    fontSize: vp.isMobile ? 13 : 14,
                    color: '#C9A97A',
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {hours.map(({ day, times, closed, note, highlight }, i) => (
              <div key={day} style={{
                display: 'flex',
                justifyContent: 'space-between',
                alignItems: 'flex-start',
                ...
                <div style={{
                  ...
                  letterSpacing: '0.01em',
                  minWidth: 90,
                }}>{lang === 'vec' ? DAY_NAMES_VEC[DAY_NAMES_IT.indexOf(day)] : en ? DAY_NAMES_EN[DAY_NAMES_IT.indexOf(day)] : day}</div>
                <div style={{ textAlign: 'right' }}>
                  ...
                  {note && (
                    <div style={{
                      ...
                      color: '#6B4C33',
                      marginTop: 2,
                    }}>{lang === 'vec' ? 'Serà de basora' : en ? 'Closed in the afternoon' : note}</div>
                  )}
                </div>
              </div>
            ))}
      ...
      {/* General-inquiry form — NOT for orders */}
      <ScrollReveal style={{ marginTop: vp.isMobile ? 40 : 64 }}>
      <div id="inquiry-form" style={{ background: '#FDFAF4', borderRadius: 16, padding: vp.isMobile ? '32px 22px' : '44px 48px', boxShadow: '0 2px 12px rgba(26,17,8,0.08)', border: '1px solid rgba(26,17,8,0.06)', scrollMarginTop: 80 }}>
        <div style={{ display: 'grid', gridTemplateColumns: vp.isMobile ? '1fr' : '1fr 1.2fr', gap: vp.isMobile ? 24 : 48, alignItems: 'start' }}>
          <div>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 12 }}>{lang === 'vec' ? 'Informazion e curiosità' : en ? 'Info & Questions' : 'Informazioni e curiosità'}</div>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: vp.isPhone ? 24 : vp.isMobile ? 28 : 32, fontWeight: 700, color: '#1A1108', marginBottom: 14, lineHeight: 1.2, letterSpacing: '-0.01em' }}>{lang === 'vec' ? "Gheto 'na domanda?" : en ? 'Have a Question?' : 'Hai una domanda?'}</h2>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 14 : 15, color: '#3D2B1A', lineHeight: 1.75, marginBottom: 16 }}>
              {lang === 'vec' ? <React.Fragment>Scrìvine par <strong>informazion generali</strong>, consigli o curiosità su la nostra storia e la nostra roba.</React.Fragment> : en ? <React.Fragment>Write to us for <strong>general information</strong>, suggestions or questions about our history and products.</React.Fragment> : <React.Fragment>Scrivici per <strong>informazioni generali</strong>, suggerimenti o curiosità sulla nostra storia e i nostri prodotti.</React.Fragment>}
            </p>
            <p style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 13, color: '#6B4C33', lineHeight: 1.6 }}>
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#3D2B1A', lineHeight: 1.7, marginTop: 18 }}>
              {lang === 'vec' ? 'O scrìvine da la to posta:' : en ? 'Or write from your own mail:' : 'Oppure scrivici dalla tua posta:'}{' '}
              <a href={'mailto:' + CONTACT_EMAIL} style={{ color: '#C8251D', fontWeight: 600, textDecoration: 'none', borderBottom: '1px solid rgba(200,37,29,0.35)' }}>{CONTACT_EMAIL}</a>
            </p>
          </div>
          <div>
            {sent ? (
              <div style={{ background: '#F5F0E6', borderRadius: 14, padding: '36px 28px', textAlign: 'center', border: '1px solid rgba(26,17,8,0.06)', animation: 'contact-success-pop 1s cubic-bezier(0.16,1,0.3,1)' }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 600, color: '#1A1108', marginBottom: 8 }}>{lang === 'vec' ? 'Grazie!' : en ? 'Thank you!' : 'Grazie!'}</div>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: '#6B4C33', lineHeight: 1.6 }}>{lang === 'vec' ? <React.Fragment>Gavémo ciapà el to mesajo.<br/>Te rispondémo pì presto che podémo.</React.Fragment> : en ? <React.Fragment>We've received your message.<br/>We'll get back to you soon.</React.Fragment> : <React.Fragment>Abbiamo ricevuto il tuo messaggio.<br/>Ti risponderemo al più presto.</React.Fragment>}</p>
              </div>
            ) : (
              <form onSubmit={async e => {
                e.preventDefault();
                const f = e.currentTarget, d = new FormData(f);
                /* Se la ciave de Web3Forms no la xe ancora messa (vedi CONTACT_FORM_KEY qua sora),
                   se verze el programa de posta del visitador: el modulo funsiona istesso. */
                if (!CONTACT_FORM_KEY) {
                ...
                setSending(true);
                try {
                  const send = k => fetch('https://api.web3forms.com/submit', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify({ access_key: k, subject: 'Sito Piva — ' + (d.get('name') || 'nuovo messaggio'), from_name: d.get('name'), name: d.get('name'), email: d.get('email'), message: d.get('message') }) });
                  const results = await Promise.all([send(CONTACT_FORM_KEY), CONTACT_FORM_KEY_2 ? send(CONTACT_FORM_KEY_2) : Promise.resolve({ ok: true })]);
                  if (!results[0].ok) throw new Error();
                  setSent(true);
                } catch (err) { setSendError(true); } finally { setSending(false); }
              }}>
                <div className="field"><label>{lang === 'vec' ? 'Nome e Cognome' : en ? 'Full Name' : 'Nome e Cognome'}</label><input name="name" type="text" placeholder={en ? 'John Smith' : 'Mario Rossi'} required /></div>
                <div className="field"><label>Email</label><input name="email" type="email" placeholder={en ? 'john@example.com' : 'mario@esempio.it'} required /></div>
                <div className="field"><label>{lang === 'vec' ? 'Messajo' : en ? 'Message' : 'Messaggio'}</label><textarea name="message" rows="4" placeholder={lang === 'vec' ? 'Scrivi qua la to domanda…' : en ? 'Write your question here…' : 'Scrivi qui la tua domanda…'} required></textarea></div>
                <button type="submit" disabled={sending} style={{
                  background: '#C8251D', color: '#fff', fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 600,
                  padding: '13px 28px', borderRadius: 999, border: 'none', cursor: 'pointer',
                  letterSpacing: '0.04em', boxShadow: '0 4px 16px rgba(200,37,29,0.30)', transition: 'all 0.25s ease',
                  width: vp.isMobile ? '100%' : undefined,
                }}
                  onMouseEnter={e => { e.currentTarget.style.background='#9E1C16'; e.currentTarget.style.transform='translateY(-1px)'; }}
                  onMouseLeave={e => { e.currentTarget.style.background='#C8251D'; e.currentTarget.style.transform='translateY(0)'; }}
                >{sending ? (lang === 'vec' ? 'Mandémo…' : en ? 'Sending…' : 'Invio…') : (lang === 'vec' ? 'Manda el mesajo' : en ? 'Send Message' : 'Invia messaggio')}</button>
                <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: '#6B4C33', marginTop: 12, fontStyle: 'italic', lineHeight: 1.5 }}>
                  {lang === 'vec' ? <React.Fragment>Modulo par <strong>informazion generali</strong>. No sta doparlo par ordini o prenotazion.</React.Fragment> : en ? <React.Fragment>Form for <strong>general information</strong>. Do not use this for orders or reservations.</React.Fragment> : <React.Fragment>Modulo per <strong>informazioni generali</strong>. Non utilizzare per ordini o prenotazioni.</React.Fragment>}
                </div>
                {sendError && (
                  <div style={{ fontFamily: 'var(--font-body)', fontSize: 12.5, color: '#C8251D', marginTop: 10, lineHeight: 1.6 }}>
                    {lang === 'vec' ? 'No semo riussìi a mandar el mesajo. Prova da novo, o scrìvine a ' : en ? "We couldn't send your message. Please try again, or write to " : 'Non siamo riusciti a inviare il messaggio. Riprova, oppure scrivici a '}
                    <a href={'mailto:' + CONTACT_EMAIL} style={{ color: '#C8251D', fontWeight: 600 }}>{CONTACT_EMAIL}</a>
                  </div>
                )}
              </form>
            )}
...
/* Immagini in stagione per una sezione del menù: piatti senza stagione (tutto
   l'anno) più quelli la cui stagione coincide con quella corrente; esclude i
   piatti fuori stagione. */
function getSectionInSeasonImgs(sectionId) {
  const dishByName = {};
  MENU_DISHES.forEach(d => { dishByName[d.name] = d; });
  ...
  return section.dishes
    .map(n => dishByName[n])
    .filter(Boolean)
    .filter(d => !d.season || d.season === season)
    .map(d => ({ src: d.src, name: d.name }));
}

/* Immagine che ruota (dissolvenza) tra i piatti in stagione della sezione. */
function RotatingTileImg({ imgs, fallback, alt, height }) {
  const list = (imgs && imgs.length) ? imgs : [{ src: fallback, name: alt }];
  ...
  return (
    <div ref={ref} style={{ position: 'relative', width: '100%', height, background: '#fff', overflow: 'hidden' }}>
      {list.map((it, i) => (
        <img key={it.src + i} src={it.src} alt={it.name || alt} loading="lazy" decoding="async"
          style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', display: 'block', opacity: i === (idx % list.length) ? 1 : 0, transition: 'opacity 0.6s ease' }} />
      ))}
      {list.length > 1 && (
        <div style={{ position: 'absolute', bottom: 8, left: 0, right: 0, display: 'flex', justifyContent: 'center', gap: 5 }}>
          {list.map((_, i) => (
            <span key={i} style={{ width: 5, height: 5, borderRadius: '50%', background: i === (idx % list.length) ? '#FDFAF4' : 'rgba(253,250,244,0.45)', boxShadow: '0 0 2px rgba(0,0,0,0.4)' }} />
          ))}
        </div>
      )}
    </div>
...
/* Animated stat: counts up from 0 to its numeric value the first time it scrolls into view.
   Preserves any non-numeric suffix (e.g. the "+" in "200+"). */
function AnimatedStat({ n, l, numStyle }) {
  ...
  return (
    <div className="stat" ref={ref}>
      <div className="stat-num" style={numStyle}>{display}</div>
...
function HomePage({ onNavigate }) {
  ...
  const ricetteImgs = React.useMemo(() => {
    const season = getSeasonForDate(new Date());
    const inSeason = RECIPES.filter(r => !r.season || r.season === season).map(r => ({ src: r.img, name: r.name }));
    return [{ src: window.IMGS.piattiStagionali, name: 'Ricette' }, ...inSeason];
  }, []);
  const foodImgs = [
    { rotate: primiImgs, fallback: window.IMGS.pastaFresca, label: lang === 'vec' ? 'Primi Piati' : en ? 'First Courses' : 'Primi Piatti', anchor: 'section-primi' },
    { rotate: secondiImgs, fallback: window.IMGS.ricetteFeste, label: lang === 'vec' ? 'Secondi Piati' : en ? 'Main Courses' : 'Secondi Piatti', anchor: 'section-secondi' },
    { rotate: prodottiImgs, fallback: window.IMGS.prodottiQualita, label: lang === 'vec' ? 'Ròba Bòna' : en ? 'Quality Products' : 'Prodotti di qualità', anchor: 'section-prodotti' },
    { rotate: ricetteImgs, fallback: window.IMGS.piattiStagionali, label: lang === 'vec' ? 'Ricéte' : en ? 'Recipes' : 'Ricette', page: 'ricette' },
  ];
  ...
  return (
    <>
      ...
      {/* About strip */}
      <div style={{ background: '#FDFAF4', padding: vp.isMobile ? '48px 20px' : '80px 40px' }}>
        <RevealSection storageKey="piva_hero_assembled" style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: vp.isMobile ? '1fr' : '1fr 1fr', gap: vp.isMobile ? 32 : 64, alignItems: 'center' }}>
          <div>
            <SlideInLine index={0}><div style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 10 : 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 14 }}>{lang === 'vec' ? 'La storia nostra' /*kicker*/ : en ? 'Our Story' : 'La nostra storia'}</div></SlideInLine>
            <SlideInLine index={1}><h2 style={{ fontFamily: 'var(--font-display)', fontSize: vp.isPhone ? 26 : vp.isMobile ? 30 : 36, fontWeight: 700, color: '#1A1108', lineHeight: 1.2, marginBottom: 18 }}>{lang === 'vec' ? 'Granda profesionalità e tanta cortesia' : en ? 'Great Professionalism and Kindness' : 'Grande professionalità e gentilezza'}</h2></SlideInLine>
            <SlideInLine index={2}><p style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 14 : 15, color: '#3D2B1A', lineHeight: 1.8, marginBottom: 28 }}>{lang === 'vec' ? "Da do generazion, la fameia Piva la porta in tòla i savori veri del Polesine. Da la salumeria in scartoso infina a la gastronomia, ogni piato el xe fato co' roba fresca sielta tute le matine, prima che el sol el sia in pìe!" : en ? 'For two generations, the Piva family has brought the authentic flavors of the Polesine to the table. From the delicatessen to the prepared foods, every dish is made with fresh ingredients selected every morning.' : 'Da due generazioni, la famiglia Piva porta in tavola i sapori autentici del Polesine. Dalla salumeria alla gastronomia, ogni piatto è preparato con ingredienti freschi selezionati ogni mattina.'}</p></SlideInLine>
            <SlideInLine index={3}><button onClick={() => onNavigate('about')} style={{
              background: 'transparent', color: '#C8251D', fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600,
              padding: '10px 22px', borderRadius: 999, border: '1.5px solid #C8251D', cursor: 'pointer',
              letterSpacing: '0.04em', transition: 'all 0.25s ease',
            }}
              onMouseEnter={e => e.currentTarget.style.background='#F9EAEA'}
              onMouseLeave={e => e.currentTarget.style.background='transparent'}
            >{lang === 'vec' ? 'Varda la nostra storia' : en ? 'Discover Our Story' : 'Scopri la nostra storia'}</button></SlideInLine>
          </div>
          <div style={{ borderRadius: 12, overflow: 'hidden', background: '#FDFAF4', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <AssembleImage src={window.IMGS.stillLife} alt="Selezione di specialità F.lli Piva — prosciutto di Parma, tortellini, aceto balsamico e conserve" />
          </div>
        </RevealSection>
      </div>

      {/* Menu preview */}
      <div style={{ padding: vp.isMobile ? '48px 20px' : '72px 40px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: vp.isMobile ? 28 : 40 }}>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 10 : 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#C8251D', marginBottom: 10 }}>{lang === 'vec' ? 'Ogni dì freschi' : en ? 'Fresh Every Day' : 'Ogni giorno freschi'}</div>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: vp.isPhone ? 26 : vp.isMobile ? 30 : 36, fontWeight: 700, color: '#1A1108' }}>{lang === 'vec' ? 'I nostri piati e la nostra roba' : en ? 'Our Dishes and Products' : 'Nostri piatti e prodotti'}</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${previewCols}, 1fr)`, gap: vp.isMobile ? 12 : 16, marginBottom: vp.isMobile ? 28 : 40 }}>
          {foodImgs.map(({ src, rotate, fallback, label, anchor, page }) => (
            <div key={label} onClick={() => onNavigate(page || 'menu', anchor)} style={{ borderRadius: 14, overflow: 'hidden', boxShadow: '0 4px 16px rgba(26,17,8,0.12)', cursor: 'pointer', transition: 'transform 0.25s ease' }}
              onMouseEnter={e => e.currentTarget.style.transform='translateY(-4px)'}
              onMouseLeave={e => e.currentTarget.style.transform='translateY(0)'}
            >
              {rotate
                ? <RotatingTileImg imgs={rotate} fallback={fallback} alt={label} height={vp.isMobile ? 140 : 180} />
                : <img src={src} alt={label} loading="lazy" decoding="async" style={{ width: '100%', height: vp.isMobile ? 140 : 180, objectFit: 'cover', display: 'block' }} />}
              <div style={{ padding: vp.isMobile ? '8px 10px' : '10px 14px', background: '#FDFAF4' }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: vp.isMobile ? 12 : 13, fontWeight: 600, color: '#1A1108' }}>{label}</div>
              </div>
            </div>
          ))}
        </div>
        <div style={{ textAlign: 'center' }}>
          <button onClick={() => onNavigate('menu')} style={{
            ...
            letterSpacing: '0.04em', boxShadow: '0 4px 20px rgba(200,37,29,0.30)', transition: 'all 0.25s ease',
          }}
            onMouseEnter={e => { e.currentTarget.style.background='#9E1C16'; e.currentTarget.style.transform='translateY(-2px)'; }}
            onMouseLeave={e => { e.currentTarget.style.background='#C8251D'; e.currentTarget.style.transform='translateY(0)'; }}
          >{lang === 'vec' ? 'Varda tuto el menù' : en ? 'See the Full Menu' : 'Vedi il menù completo'}</button>
        </div>
      </div>

      {/* Gift banner */}
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        <img src={window.IMGS.cesteBg} alt="Ceste regalo" loading="lazy" decoding="async" style={{ width: '100%', height: vp.isMobile ? 220 : 300, objectFit: 'cover', display: 'block' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(26,17,8,0.60)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: vp.isMobile ? 10 : 14, padding: '0 20px', textAlign: 'center' }}>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: vp.isMobile ? 10 : 11, fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#D4A640' }}>{lang === 'vec' ? "'Na tradizion che va vanti" : en ? 'A Tradition That Continues' : 'Una tradizione che continua'}</div>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: vp.isPhone ? 26 : vp.isMobile ? 30 : 36, fontWeight: 700, color: '#FDFAF4', textAlign: 'center', maxWidth: 720 }}>{lang === 'vec' ? 'Pachéti Regalo & Ceste de Nadal' : en ? 'Gift Packages & Holiday Baskets' : 'Confezioni Regalo & Ceste Natalizie'}</h2>
          <p style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: vp.isMobile ? 14 : 16, color: '#E8DCC8', maxWidth: 560 }}>{lang === 'vec' ? "Composizion curàe co' la nostra roba, giuste par ogni ocasión." : en ? 'Curated compositions with our products, perfect for any occasion.' : 'Composizioni curate con i nostri prodotti, perfette per ogni occasione.'}</p>
        </div>
      </div>
    </>
  );
...
/* ─────────────────── BACKGROUND MUSIC (revert: see MUSIC-NOTES.md) ─────────────────── */
const MUSIC_TRACKS = [
  { src: 'audio/inno-italia-navy-band.mp3', title: "Inno d'Italia", artist: 'U.S. Navy Band' },
];
const MUSIC_MIME = 'audio/mpeg';   // real container type of the files above
// The host may serve these files with a wrong Content-Type, which makes some browsers
// refuse to decode them. We fetch each track and re-wrap it in a Blob with the correct
// MIME so playback never depends on the server's guess.
const MUSIC_PLAYS_PER_TRACK = 1;   // each track plays this many times before moving on
const MUSIC_NO_REPEAT_WINDOW = 7;  // a track can't come back until 7 others have played
/* Random order with memory: pick uniformly among tracks not in the recent-history window. */
...
const MUSIC_FADE_MS = 5000;   // fade-out tail
const MUSIC_GAP_MS = 1000;    // silence before the loop restarts
function MusicPlayer() {
  ...
  /* Richiamo al primo accesso: pastiglia "clicca" + bordo pulsante finché
     l'utente non tocca il tasto la prima volta (una volta per visita: usa
     sessionStorage, quindi ricompare a ogni nuova sessione del browser).
     Revert: togliere invite/dismissInvite e il blocco <div> della pastiglia. */
  const [invite, setInvite] = useState(() => {
    try { return !sessionStorage.getItem('piva_music_seen'); } catch (e) { return true; }
  });
  const dismissInvite = () => {
    setInvite(false);
    try { sessionStorage.setItem('piva_music_seen', '1'); } catch (e) {}
  };
  const loadTrack = React.useCallback(i => {
    return fetch(MUSIC_TRACKS[i].src)
      .then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.arrayBuffer(); })
      .then(buf => URL.createObjectURL(new Blob([buf], { type: MUSIC_MIME })));
  }, []);
  useEffect(() => {
    let alive = true, url = '';
    ...
    setCurrent(first);
    loadTrack(first).then(u => { if (!alive) { URL.revokeObjectURL(u); return; } url = u; setSrcUrl(u); })
      .catch(() => {});
  ...
  const fadeTo = (target, done) => {
    ...
    const step = 60, from = a.volume, delta = target - from;
    let elapsed = 0;
    fadeRef.current = setInterval(() => {
      elapsed += step;
      const p = Math.min(1, elapsed / MUSIC_FADE_MS);
      a.volume = Math.max(0, Math.min(1, from + delta * p));
      if (p >= 1) { clearInterval(fadeRef.current); fadeRef.current = null; if (done) done(); }
    }, step);
  };
  /* Fade the tail out as each track approaches its end. */
  const onTimeUpdate = () => {
    const a = audioRef.current;
  ...
  /* On end: hold a beat of silence, then fade the next play in.
     Each track repeats MUSIC_PLAYS_PER_TRACK times before advancing. */
  const onEnded = () => {
    const a = audioRef.current;
    if (!a) return;
    clearTimers();
      ...
      if (MUSIC_TRACKS.length > 1) {
        loadTrack(trackRef.current).then(u => {
          ...
          gapRef.current = setTimeout(() => {
            try { a.volume = MUSIC_VOLUME; } catch (e) {}
  ...
  const toggle = () => {
    ...
    if (!a) return;
    dismissInvite();
    if (playing) { clearTimers(); a.pause(); return; }
    setHint('');
    try { a.volume = MUSIC_VOLUME; } catch (e) {}
    const p = a.play();
  ...
  const label = playing
    ? (lang === 'vec' ? 'Smorza la mùsica' : lang === 'en' ? 'Mute music' : 'Silenzia la musica')
    : (lang === 'vec' ? 'Impìsa la mùsica' : lang === 'en' ? 'Play music' : 'Ascolta la musica');
  ...
  return (
    <React.Fragment>
      <audio ref={audioRef} src={srcUrl} preload="auto" playsInline
        onPlay={() => { setPlaying(true); setHint(''); }} onPause={() => setPlaying(false)}
        onTimeUpdate={onTimeUpdate} onEnded={onEnded}
        onError={() => { const a = audioRef.current; setHint('audio error ' + ((a && a.error && a.error.code) || '?')); }}></audio>
      {hint && (
        <div role="status" style={{
          position: 'fixed', left: vp.isMobile ? 14 : 20, bottom: vp.isMobile ? 108 : 118, zIndex: 200, maxWidth: 210,
          background: '#1A1108', color: '#F5F0E6', borderRadius: 10, padding: '9px 12px',
          fontFamily: 'var(--font-body)', fontSize: 11.5, lineHeight: 1.5,
          boxShadow: '0 8px 24px rgba(26,17,8,0.28)',
        }}>{hintText}<div style={{ marginTop: 6, opacity: 0.7, fontSize: 10 }}>{hint}</div></div>
      )}
      {playing && (
        <div aria-live="polite" style={{
          position: 'fixed', left: vp.isMobile ? 14 : 20, bottom: vp.isMobile ? 64 : 70, zIndex: 200, maxWidth: vp.isMobile ? 210 : 250,
          background: '#1A1108', color: '#F5F0E6', borderRadius: 10, padding: '7px 11px',
          border: '1px solid rgba(212,166,64,0.4)', boxShadow: '0 6px 18px rgba(26,17,8,0.24)',
          fontFamily: 'var(--font-body)', fontSize: 11.5, lineHeight: 1.35,
        }}>
          <div style={{ fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#D4A640', marginBottom: 2 }}>
            {lang === 'vec' ? 'Sonàndo' : lang === 'en' ? 'Now playing' : 'In riproduzione'}
          </div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 13.5 }}>{track.title}</div>
          {track.artist && <div style={{ opacity: 0.65, fontSize: 10.5, marginTop: 1 }}>{track.artist}</div>}
        </div>
      )}
      <div style={{
        position: 'fixed', left: vp.isMobile ? 14 : 20, bottom: vp.isMobile ? 14 : 20, zIndex: 200,
        display: 'flex', alignItems: 'center', gap: 8,
      }}>
      <button onClick={toggle} aria-label={label} title={label} style={{
        width: 44, height: 44, borderRadius: '50%', background: playing ? '#C8251D' : invite ? '#C8251D' : '#1A1108',
        color: '#FDFAF4', border: '1.5px solid rgba(212,166,64,0.55)', cursor: 'pointer',
        boxShadow: '0 4px 16px rgba(26,17,8,0.28)', display: 'flex', alignItems: 'center', justifyContent: 'center',
        transition: 'background 0.25s ease', flex: '0 0 auto',
        animation: invite ? 'music-invite 2.2s ease-in-out infinite' : 'none',
      }}>
        {playing ? (
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M11 5 6 9H3v6h3l5 4V5Z"/><path d="M22 9l-6 6M16 9l6 6"/></svg>
        ) : (
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M11 5 6 9H3v6h3l5 4V5Z"/><path d="M16 8.5a4 4 0 0 1 0 7M19 6a7 7 0 0 1 0 12"/></svg>
        )}
      </button>
      {invite && (
        <div aria-hidden="true" style={{
          background: '#C8251D', color: '#FDFAF4', borderRadius: 999, padding: '6px 12px',
          fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase',
          boxShadow: '0 4px 16px rgba(200,37,29,0.32)', whiteSpace: 'nowrap', pointerEvents: 'none',
          animation: 'music-invite-pill 2.2s ease-in-out infinite',
        }}>{lang === 'vec' ? 'Sc-iòca par la mùsica' : lang === 'en' ? 'Click for music' : 'Clicca per la musica'}</div>
      )}
      </div>
    </React.Fragment>
  );
...
function App() {
  ...
  /* La lingua parte sempre dall'italiano a ogni visita (nessuna memoria fra sessioni). */
  ...
  // Runs after React has committed the new page's DOM, so the target page's
  // real height/layout is in place before we touch scroll position — fixes
  // scroll-anchoring pulling the viewport back down on shorter pages.
  useEffect(() => {
  ...
  return (
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <LangBar onNavigate={navigate} />
      <DialectMarquee />
      <main style={{ flex: 1 }}>
        {page === 'home' && <HomePage onNavigate={navigate} />}
        {page === 'menu' && <MenuSection />}
        {page === 'about' && <AboutPage />}
        {page === 'contact' && <ContactPage />}
        {page === 'ricette' && <RecipePage />}
      </main>
